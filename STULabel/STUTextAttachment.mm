...
static void clampTextAttachmentParameters(STUTextAttachment *__unsafe_unretained self)
{
  ...
  self->_colorInfo = self->_colorInfo & (STUTextAttachmentIsGrayscale | STUTextAttachmentUsesExtendedColors);
}
...
static void initCommon(STUTextAttachment *__unsafe_unretained self)
{
  ...
  STU_STATIC_CONST_ONCE(
      DrawMethod,
      trivialDrawMethod,
      (DrawMethod)class_getInstanceMethod(STUTextAttachment.class, @selector(drawInContext:imageBounds:)));
...
#define FOR_ALL_FIELD_NAMES(f)                                                                                         \
  f(width) f(ascent) f(descent) f(leading) f(imageBounds) f(colorInfo) f(stringRepresentation)
...
{
  return CGRect{{0, -(_ascent + _leading / 2)}, {_width, _ascent + _descent + _leading}};
}
- (CGRect)imageBounds
{
  return _imageBounds;
                               ...
                               DrawingContext &context)
{
  ...
  const auto guard = ScopeGuard{[&] { CGContextScaleCTM(cgContext, 1, -1); }};
  ...
  for (Int i = 0; i < glyphCount; ++i, origin.x += self->_width) {
    self->_drawMethod(
        self, @selector(drawInContext:imageBounds:), cgContext, CGRect{origin, self->_imageBounds.size()});
  }
                             ...
                             stringRepresentation:(nullable NSString *)stringRepresentation
{
  ...
  self = [self initWithImage:image
              ...
              verticalOffset:-bounds.origin.y
                     padding:UIEdgeInsetsZero
                     leading:0
        stringRepresentation:stringRepresentation];
...
- (nonnull instancetype)initWithWidth:(CGFloat __unused)width
                 ...
                 stringRepresentation:(nullable NSString *__unused)stringRepresentation NS_UNAVAILABLE
{
  [self doesNotRecognizeSelector:_cmd];
  __builtin_trap();
}
...
+ (instancetype)stu_newWithSTUAttachment:(nonnull STUTextAttachment *)attachment NS_RETURNS_RETAINED
{
  return [[self alloc] initWithString:@"\uFFFC"
                           attributes:@{
                             ...
                             (__bridge NSString *)kCTRunDelegateAttributeName : [attachment newCTRunDelegate]
                           }];
}
...
template <int n>
static void
addRange(Vector<AttachmentRange, n> &ranges, STUTextAttachment *__unsafe_unretained attachment, NSRange range)
{
  if (!ranges.isEmpty() && ranges[$ - 1].attachment == attachment && ranges[$ - 1].end() == range.location) {
...
static void addRunDelegatesIfNecessary(NSAttributedString *__unsafe_unretained attributedString,
                                       ...
                                       ArrayRef<const AttachmentRange> ranges)
{
  ...
  for (const AttachmentRange &range : ranges) {
    ...
    if ([attributedString attribute:fixForRDAR36622225AttributeName atIndex:range.location effectiveRange:nil]) {
      initializeNewAttributedStringIfNeccessary();
      [newAttributedString removeAttribute:fixForRDAR36622225AttributeName range:NSRange{range.location, 1}];
    }
    for (UInt i = 1; i < range.length; ++i) {
      ...
      if (!value || effectiveRange.length > 1 ||
          ((void)CFNumberGetValue((__bridge CFNumberRef)value, kCFNumberNSIntegerType, &n), n != sign_cast(i))) {
        initializeNewAttributedStringIfNeccessary();
        [newAttributedString addAttribute:fixForRDAR36622225AttributeName
                                    value:@(i)
                                    range:NSRange{range.location + i, 1}];
      }
    }
  }
}
...
- (NSAttributedString *)stu_attributedStringByConvertingNSTextAttachmentsToSTUTextAttachments
{
  ...
  [self
      enumerateAttributesInRange:NSRange{0, self.length}
                         options:NSAttributedStringEnumerationLongestEffectiveRangeNotRequired
                      usingBlock:^(NSDictionary<NSAttributedStringKey, id> *__unsafe_unretained attributes,
                                   NSRange range,
                                   BOOL *shouldStop __unused) {
                        ...
                        if (!attachment)
                          return;
                        auto *const imageAttachment =
                            [[STUImageTextAttachment alloc] initWithNSTextAttachment:attachment
                        ...
                        [newAttributedString addAttributes:@{
                          STUAttachmentAttributeName : imageAttachment,
                          (__bridge NSAttributedStringKey)
                          kCTRunDelegateAttributeName : [imageAttachment newCTRunDelegate]
                        }
                                                     range:range];
                        for (UInt i = 1; i < range.length; ++i) {
                          [newAttributedString addAttribute:fixForRDAR36622225AttributeName
...
- (NSAttributedString *)stu_attributedStringByAddingCTRunDelegatesForSTUTextAttachments
{
  ...
  [self enumerateAttribute:STUAttachmentAttributeName
                ...
                usingBlock:^(id value, NSRange range, BOOL *) {
                  if (!value)
                    return;
                  if (![value isKindOfClass:stuTextAttachmentClass])
                    return;
                  ...
                  for (const UInt indexWithoutOffset : Range{stringRange}.iter()) {
                    ...
                    if (stringRepresention != nil) {
                      ...
                      if (index == 0 || index + 1 == length + indexOffset ||
                          ...
                          (isUnicodeWhitespace([newString characterAtIndex:index + 1]) ||
                           [newAttributedString attribute:STUAttachmentAttributeName
                                                  atIndex:index + 1
                                           effectiveRange:nil])) {
                        n = 0;
                      } else {
                    ...
                    if (n == 0) {
                      ...
                      if (STU_LIKELY(index != 0)) {
                        ...
                        if (indexAfterAttachments != 0) {
                          attributes = [newAttributedString attributesAtIndex:indexAfterAttachments effectiveRange:nil];
                        } else {
