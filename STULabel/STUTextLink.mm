...
static STU_INLINE Class stuTextLinkClass()
{
  STU_STATIC_CONST_ONCE(Class, value, STUTextLink.class);
  return value;
}

static STUTextLink *__nonnull STUTextLinkCreateCreate(__unsafe_unretained id attributeValue,
...
{
  auto *const link =
      static_cast<STUTextLink *>(STUTextRectArrayCreate(stuTextLinkClass(), spans, lines, origin, scaleFactors));
...
static STUTextLink *__nonnull STUTextLinkCopyWithTextFrameOriginOffset(
    const STUTextLink *__unsafe_unretained link, CGPoint textFrameOriginOffset) NS_RETURNS_RETAINED
{
  auto *const newLink =
      static_cast<STUTextLink *>(STUTextRectArrayCopyWithOffset(stuTextLinkClass(), link, textFrameOriginOffset));
...
{
  STU_STATIC_CONST_ONCE(STUTextLinkArrayWithOriginalTextFrameOrigin *,
                        instance,
...
STU_INLINE
ArrayRef<STUTextLink * __unsafe_unretained> links(const STUTextLinkArrayWithOriginalTextFrameOrigin *self)
{
...
IntervalSearchTable verticalSearchTable(const STUTextLinkArrayWithOriginalTextFrameOrigin *self)
{
  ...
  const Float32 *const maxYs = reinterpret_cast<const Float32 *>(static_cast<const void *>(array.end()));
  return {ArrayRef{maxYs, array.count()}, ArrayRef{maxYs + array.count(), array.count()}};
}

STUTextLinkArrayWithTextFrameOrigin *__nonnull STUTextLinkArrayCreateWithTextFrameOriginAndDisplayScale(
    const TextFrame &textFrame,
    CGPoint frameOrigin,
    const TextFrameScaleAndDisplayScale &scaleFactors) NS_RETURNS_RETAINED
{
  ...
  const TaggedRangeLineSpans rls = findAndSortTaggedRangeLineSpans(
      ...
      [](const TextStyle &style) -> UInt { return reinterpret_cast<UInt>(style.linkInfo()->attribute); },
      [](UInt tag1, UInt tag2) -> bool {
        return [(__bridge id) reinterpret_cast<void *>(tag1) isEqual:(__bridge id) reinterpret_cast<void *>(tag2)];
      });

  const Int count = rls.spanTagCount;

  STUTextLinkArrayWithOriginalTextFrameOrigin *const instance = stu_createClassInstance(
      STUTextLinkArrayWithOriginalTextFrameOrigin.class, sign_cast(count) * (sizeof(void *) + 2 * sizeof(Float32)));
  ...
  Int32 linkIndex = 0;
  rls.forEachTaggedLineSpanSequence([&](ArrayRef<const TextLineSpan> spans,
                                        FirstLastRange<const TaggedStringRange &> ranges) {
    ...
    if (!paragraphs[ranges.first.paragraphIndex].excisedRangeInOriginalString().contains(rangeInOriginalString.start)) {
      const TextStyle *style = ranges.first.nonOverriddenStyle();
      for (;;) {
        const TextStyle *previous = &style->previous();
        if (style == previous)
          break;
        if (!previous->hasLink() || !equal(linkValue, previous->linkInfo()->attribute))
          break;
        style = previous;
      }
      rangeInOriginalString.start = style->stringIndex();
    }
    if (!paragraphs[ranges.last.paragraphIndex].excisedRangeInOriginalString().contains(rangeInOriginalString.end -
                                                                                        1)) {
    ...
    STUTextLink *const link = STUTextLinkCreateCreate(linkValue,
...
static Optional<Int>
indexOfLinkClosestToPoint(const STUTextLinkArrayWithOriginalTextFrameOrigin *self, CGPoint point, CGFloat maxDistance)
{
  const ArrayRef<STUTextLink * __unsafe_unretained> array = links(self);
  const Range<Int> indexRange = verticalSearchTable(self).indexRange(
      {narrow_cast<Float32>(point.y - maxDistance), narrow_cast<Float32>(point.y + maxDistance)});
  Optional<Int> minIndex = none;
  for (const Int i : indexRange.iter()) {
    const STUIndexAndDistance r = STUTextRectArrayFindRectClosestToPoint(array[i], point, maxDistance);
    if (r.index == NSNotFound || (r.distance == maxDistance && minIndex))
      continue;
    minIndex = i;
...
static Optional<Int> indexOfLinkMatching(const STUTextLinkArrayWithOriginalTextFrameOrigin *__unsafe_unretained self,
                                         id attributeValue,
                                         Range<Int32> rangeInOriginalString,
                                         Range<Int32> rangeInTruncatedString)
{
  ...
  const Range<Int> indexRange = {
      binarySearchFirstIndexWhere(
          array, [&](const STUTextLink *p) { return p->_rangeInOriginalString.end > rangeInOriginalString.start; })
          .indexOrArrayCount,
  ...
  for (const Int i : indexRange.iter()) {
    ...
    if (!rangeInOriginalString.overlaps(other._rangeInOriginalString))
      continue;
    if (!equal(attributeValue, other._linkAttributeValue))
      continue;
...
{
  if (attributeValue) {
    if (const Optional<Int> index = indexOfLinkMatching(self,
                                                        attributeValue,
...
- (nullable STUTextLink *)linkClosestToPoint:(CGPoint)point maxDistance:(CGFloat)maxDistance
{
...
STUTextLinkArrayCopyWithShiftedTextFrameOrigin(const STUTextLinkArrayWithTextFrameOrigin *__unsafe_unretained oldArray,
                                               CGPoint newTextFrameOrigin) NS_RETURNS_RETAINED
{
  STU_STATIC_CONST_ONCE(
      Class, textLinkArrayWithShiftedTextFrameOriginClass, STUTextLinkArrayWithShiftedTextFrameOrigin.class);
  ...
  STUTextLinkArrayWithShiftedTextFrameOrigin *const array = stu_createClassInstance(
      textLinkArrayWithShiftedTextFrameOriginClass, sizeof(std::atomic<void *>) * sign_cast(count));
...
- (STUTextLink *)objectAtIndexedSubscript:(size_t)index
{
  ...
  STUTextLink *const link = STUTextLinkCopyWithTextFrameOriginOffset(links(_oldLinks).begin()[index], offset);
  void *value = nullptr;
  if (ap.compare_exchange_strong(value, (__bridge void *)link, std::memory_order_release, std::memory_order_acquire)) {
...
{
  if (attributeValue) {
    const Optional<Int> index = indexOfLinkMatching(_oldLinks,
                                                    attributeValue,
                                                    clampToInt32IndexRange(rangeInOriginalString),
                                                    clampToInt32IndexRange(rangeInTruncatedString));
    if (index) {
