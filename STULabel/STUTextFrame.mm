...
{
  const UInt start = range.start.indexInTruncatedString + range.start.isIndexOfInsertedHyphen;
  if (range.end <= range.start) {
    return {.location = start, .length = 0};
  }
  const UInt end = range.end.indexInTruncatedString + range.end.isIndexOfInsertedHyphen;
...
Unretained<STUTextFrame * __nonnull> stu_label::emptySTUTextFrame()
{
  STU_STATIC_CONST_ONCE(
      ...
      STUTextFrameCreateWithShapedStringRange(
          nil,
          [STUShapedString emptyShapedStringWithDefaultBaseWritingDirection : STUWritingDirectionLeftToRight],
          NSRange {},
          CGSizeZero,
          0,
          nil,
                             ...
                             cancellationFlag:(nullable const STUCancellationFlag *)cancellationFlag
{
  return (id)STUTextFrameCreateWithShapedStringRange(
      self.class, shapedString, stringRange, size, displayScale, options, cancellationFlag);
}
...
{
  return STUTextFrameCreateWithShapedStringRange(cls,
                                                 shapedString,
                                                 NSRange{0, sign_cast(shapedString->shapedString->stringLength)},
                                                 frameSize,
                                                 displayScale,
                                                 options,
                                                 nullptr);
...
STUTextFrameCreateWithShapedStringRange(__nullable Class cls,
                                        ...
                                        const STUCancellationFlag *__nullable cancellationFlag) NS_RETURNS_RETAINED
{
  if (STU_UNLIKELY(!stuShapedString))
    return nil;
  ...
  STU_CHECK_MSG(stringRange.location <= sign_cast(shapedString.stringLength) &&
                    stringRange.length <= sign_cast(shapedString.stringLength) - stringRange.location,
  ...
  TextFrameLayouter layouter{
      shapedString, Range<Int32>(stringRange), options->_options.defaultTextAlignment, cancellationFlag};
  if (layouter.isCancelled())
    return nil;
...
- (STUTextFrameLayoutInfo)layoutInfoForFrameOrigin:(CGPoint)frameOrigin displayScale:(CGFloat)displayScale
{
  ...
  return {.lineCount = tf.lineCount,
          ...
          .lastLineHeightBelowBaselineWithMinimalSpacing = tf.lastLineHeightBelowBaselineWithMinimalSpacing,
          .size = tf.size,
...
- (STUTextFrameIndex)indexForIndexInOriginalString:(size_t)index indexInTruncationToken:(size_t)indexInTruncationToken
{
  return textFrameRef(*data).index(IndexInOriginalString{index}, IndexInTruncationToken{indexInTruncationToken});
}
...
- (STUTextFrameRange)rangeOfLastTruncationToken
{
  ...
  if (tf.flags & STUTextFrameIsTruncated) {
    for (const TextFrameParagraph &para : tf.paragraphs().reversed()) {
      if (para.truncationTokenLength == 0)
        continue;
...
- (STUTextFrameGraphemeClusterRange)rangeOfGraphemeClusterClosestToPoint:(CGPoint)point
                                              ignoringTrailingWhitespace:(bool)ignoringTrailingWhitespace
                                                            ...
                                                            displayScale:(CGFloat)displayScale
{
  STU_CHECK_MSG(ignoringTrailingWhitespace, "Currently only ignoringTrailingWhitespace == true is supported.");
...
- (nonnull STUTextRectArray *)rectsForRange:(STUTextFrameRange)range frameOrigin:(CGPoint)frameOrigin
{
                               ...
                               displayScale:(CGFloat)displayScale
{
  ...
  STUTextRectArray *const array =
      STUTextRectArrayCreate(nil, spans, tf.lines(), TextFrameOrigin{frameOrigin}, scaleFactors);
  return array;
}
...
- (CGRect)imageBoundsForRange:(STUTextFrameRange)range
             ...
             cancellationFlag:(nullable const STUCancellationFlag *)cancellationFlag
{
  return STUTextFrameGetImageBoundsForRange(self, range, frameOrigin, data->displayScale, options, cancellationFlag);
}
- (CGRect)imageBoundsForRange:(STUTextFrameRange)range
                  frameOrigin:(CGPoint)frameOrigin
                 displayScale:(CGFloat)displayScale
             ...
             cancellationFlag:(nullable const STUCancellationFlag *)cancellationFlag
{
  return STUTextFrameGetImageBoundsForRange(self, range, frameOrigin, displayScale, options, cancellationFlag);
}
                              ...
                              const STUCancellationFlag *__nullable cancellationFlag)
{
  ...
  if ((!options || !options->highlightStyle()) && range == fullRange) {
    textFrame.draw(origin, context, contextBaseCTM_d, pixelAlignBaselines, options, nil, cancellationFlag);
  } else {
    TextStyleOverride styleOverride{textFrame, range, options};
    if (styleOverride.drawnRange.isEmpty())
      return;
    ...
    if (styleOverride.overrideRange.isEmpty() && styleOverride.drawnRange == fullRange) {
      ...
      textFrame.draw(origin, context, contextBaseCTM_d, pixelAlignBaselines, options, &styleOverride, cancellationFlag);
    }
  }
}
...
stu_label::Rect<CGFloat>
STUTextFrameGetImageBoundsForRange(const STUTextFrame *__unsafe_unretained self,
                                   STUTextFrameRange range,
                                   CGPoint origin,
                                   CGFloat displayScale,
                                   ...
                                   const STUCancellationFlag *__nullable cancellationFlag)
{
  ...
  ImageBoundsContext context = {.cancellationFlag = *(cancellationFlag ?: &CancellationFlag::neverCancelledFlag),
                                .drawingMode = drawingMode,
                                .displayScale = DisplayScale::create(displayScale),
                                .fontInfoCache = fontInfoCache,
                                .glyphBoundsCache = glyphBoundsCache};

  if ((!options || (!options->highlightStyle() && drawingMode == STUTextFrameDefaultDrawingMode)) &&
      range == fullRange) {
