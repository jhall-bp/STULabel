...
static void clampFirstLineOffset(InOut<CGFloat> inOutOffset, InOut<STUFirstLineOffsetType> inOutType)
{
...
#define FOR_ALL_FIELDS(f)                                                                                              \
  f(STUFirstLineOffsetType, firstLineOffsetType) f(CGFloat, firstLineOffset) f(CGFloat, minimumBaselineDistance)       \
      f(NSInteger, numberOfInitialLines) f(CGFloat, initialLinesHeadIndent) f(CGFloat, initialLinesTailIndent)
...
- (void)setFirstLineOffset:(CGFloat)firstLineOffset type:(STUFirstLineOffsetType)firstLineOffsetType
{
...
- (BOOL)isEqual:(id)object
{
  ...
  if (![object isKindOfClass:stuParagraphStyleClass])
    return false;
  STUParagraphStyle *const other = object;
#define IF_NOT_EQUAL_RETURN_FALSE(Type, name)                                                                          \
  if (!equal(_style.name, other->_style.name))                                                                         \
    return false;
...
{
  ParagraphExtraStyle &style = _style;
  const auto h = hash(style.firstLineOffsetType,
                      style.firstLineOffset,
                      style.numberOfInitialLines,
                      hashableBits(style.initialLinesHeadIndent) ^ hashableBits(style.initialLinesTailIndent));
