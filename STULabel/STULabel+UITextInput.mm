...
- (instancetype)initWithStart:(STULabelTextInputPosition*)start
                           end:(STULabelTextInputPosition*)end;
@end
...
- (STULabelTextInputPosition*)startPosition { return _startPosition; }
- (STULabelTextInputPosition*)endPosition { return _endPosition; }
- (BOOL)isEqual:(id)object {
...
- (NSWritingDirection)writingDirection { return _writingDirection; }
...
static STU_INLINE NSString* visibleString(STULabel* self) {
...
{
  if (![range isKindOfClass:STULabelTextInputRange.class]) return nil;
...
static void updateVisibleString(STULabel* self, STULabelTextInteraction* interaction) {
  ...
  if (hadSelection) [inputDelegate selectionWillChange:self];
  interaction.stu_displayedString = string;
  interaction.stu_selectedTextRange = nil;
  [inputDelegate textDidChange:self];
  if (hadSelection) [inputDelegate selectionDidChange:self];
}

static STU_INLINE STULabelTextInteraction* textInteraction(STULabel* self) {
  return [self stu_textInteraction];
}
...
static NSRange truncationTokenLinkRange(STULabel* self) {
  ...
  const NSRange tokenRange =
    STUTextFrameRangeGetRangeInTruncatedString(textFrame.rangeOfLastTruncationToken);
  if (tokenRange.length == 0) return NSMakeRange(NSNotFound, 0);
  for (STUTextLink* const link in self.links) {
    ...
    if (tokenLinkRange.length != 0) return tokenLinkRange;
  }
...
static NSRange selectionRangeExcludingTruncationTokenLink(STULabel* self, NSRange range) {
  ...
  if (rangeEnd > linkEnd) return NSMakeRange(linkEnd, rangeEnd - linkEnd);
  return NSMakeRange(NSNotFound, 0);
}

static STUTextRectArray* rectsForRange(STULabel* self, NSRange range) {
  if (range.length == 0) return STUTextRectArray.emptyArray;
  STUTextFrame* const textFrame = self.textFrame;
...
static CGRect caretRect(STULabel* self, NSUInteger index) {
  NSString* const string = visibleString(self);
  if (string.length == 0) return CGRectZero;
  ...
  if (rects.rectCount == 0) return CGRectZero;
  CGRect const rect = [rects rectAtIndex:usesPreviousCharacter ? rects.rectCount - 1 : 0];
  const NSWritingDirection direction = writingDirectionAtPoint(self, CGPointMake(CGRectGetMidX(rect),
                                                                                   CGRectGetMidY(rect)));
...
{
  STUTextFrame* const textFrame = self.textFrame;
  const STUTextFrameRange indexRange =
  ...
  if (targetLineIndex < 0 || targetLineIndex >= data->lineCount) return index;

  const STUTextFrameLine* const targetLine = STUTextFrameDataGetLines(data) + targetLineIndex;
  const NSRange targetRange = NSMakeRange((NSUInteger)targetLine->rangeInTruncatedString.start,
                                          (NSUInteger)(targetLine->rangeInTruncatedString.end
                                                     - targetLine->rangeInTruncatedString.start));
  if (targetRange.length == 0) return targetRange.location;

  const STUTextFrameLayoutInfo layoutInfo =
    [textFrame layoutInfoForFrameOrigin:self.textFrameOrigin displayScale:self.layer.contentsScale];
  const CGFloat textScaleFactor = layoutInfo.textScaleFactor > 0 ? layoutInfo.textScaleFactor : 1;
  const CGFloat x = (CGRectGetMidX(caret) - self.textFrameOrigin.x)/textScaleFactor
                  - targetLine->originX;
...
static NSUInteger positionInDirection(STULabel* self, NSString* string, NSUInteger index,
                                      UITextLayoutDirection direction)
{
  const NSWritingDirection writingDirection = writingDirectionAtPosition(self, index);
  switch (direction) {
  case UITextLayoutDirectionLeft:
    return writingDirection == NSWritingDirectionRightToLeft
         ? nextCharacterBoundary(string, index) : previousCharacterBoundary(string, index);
  case UITextLayoutDirectionRight:
    return writingDirection == NSWritingDirectionRightToLeft
         ? previousCharacterBoundary(string, index) : nextCharacterBoundary(string, index);
  case UITextLayoutDirectionUp:
  case UITextLayoutDirectionDown: {
    CGRect const rect = caretRect(self, index);
    if (CGRectIsEmpty(rect)) return index;
    return positionOnAdjacentLine(self, index, rect, direction);
  }
  }
}

static NSRange characterRangeInDirection(STULabel* self, NSString* string, NSUInteger index,
                                         UITextLayoutDirection direction)
{
  const NSUInteger otherIndex = positionInDirection(self, string, index, direction);
  if (otherIndex == index) return NSMakeRange(index, 0);
  return otherIndex < index ? NSMakeRange(otherIndex, index - otherIndex)
                            : NSMakeRange(index, otherIndex - index);
}
...
- (nullable NSString*)textInRange:(UITextRange*)range {
  ...
  if (!r) return nil;
  return [string substringWithRange:NSMakeRange(r.startPosition.index,
...
- (void)setSelectedTextRange:(nullable UITextRange*)range {
  ...
  if (newRange) {
    const NSRange normalizedRange = selectionRangeExcludingTruncationTokenLink(
      self, NSMakeRange(newRange.startPosition.index,
                        newRange.endPosition.index - newRange.startPosition.index));
    ...
    {
      newRange = [[STULabelTextInputRange alloc]
        initWithStart:textPosition(normalizedRange.location)
  ...
  if (oldRange == newRange || [oldRange isEqual:newRange]) return;
...
- (nullable UITextRange*)markedTextRange { return nil; }
- (nullable NSDictionary<NSAttributedStringKey, id>*)markedTextStyle { return nil; }
...
- (UITextPosition*)endOfDocument { return textPosition(textInputString(self).length); }
...
- (nullable UITextPosition*)positionFromPosition:(UITextPosition*)position offset:(NSInteger)offset {
  ...
  if (offset >= 0) {
    ...
    if (unsignedOffset > string.length - p.index) return nil;
...
- (NSComparisonResult)comparePosition:(UITextPosition*)position toPosition:(UITextPosition*)other {
  ...
  if (!p || !otherP) return NSOrderedSame;
  return p.index < otherP.index ? NSOrderedAscending
       : p.index > otherP.index ? NSOrderedDescending : NSOrderedSame;
}
                         ...
                         farthestInDirection:(UITextLayoutDirection)direction
{
  ...
  STULabelTextInputRange* const r = validTextRange(range, string);
  if (!r) return nil;
  const NSWritingDirection writingDirection =
    writingDirectionAtPosition(self, r.startPosition.index);
  const bool wantsStart = direction == UITextLayoutDirectionUp
...
- (void)setBaseWritingDirection:(NSWritingDirection __unused)writingDirection
                       forRange:(UITextRange* __unused)range {}

- (CGRect)firstRectForRange:(UITextRange*)range {
  NSString* const string = textInputString(self);
  STULabelTextInputRange* const r = validTextRange(range, string);
  if (!r) return CGRectZero;
  if (r.empty) return caretRect(self, r.startPosition.index);
...
- (nullable UITextPosition*)closestPositionToPoint:(CGPoint)point {
  ...
  if (range.length == 0) return textPosition(range.location);
  const CGFloat midX = CGRectGetMidX(cluster.bounds);
  const bool onTrailingHalf = cluster.writingDirection == STUWritingDirectionLeftToRight
                            ? point.x >= midX : point.x <= midX;
...
- (nullable UITextPosition*)closestPositionToPoint:(CGPoint)point withinRange:(UITextRange*)range {
  ...
  const NSUInteger index = p.index < r.startPosition.index ? r.startPosition.index
                       : p.index > r.endPosition.index ? r.endPosition.index : p.index;
  return textPosition(index);
}
                                                                 ...
                                                                 inDirection:(UITextStorageDirection __unused)direction
{
  NSString* const string = textInputString(self);
  STULabelTextInputPosition* const p = validTextPosition(position, string);
  if (!p || string.length == 0) return nil;
...
- (NSInteger)characterOffsetOfPosition:(UITextPosition*)position withinRange:(UITextRange*)range {
  ...
  STULabelTextInputPosition* const p = validTextPosition(position, string);
  STULabelTextInputRange* const r = validTextRange(range, string);
  if (!p || !r || p.index < r.startPosition.index || p.index > r.endPosition.index) return 0;
  return (NSInteger)p.index - (NSInteger)r.startPosition.index;
}

- (__kindof UIView*)textInputView { return self; }
- (UITextStorageDirection)selectionAffinity { return textInteraction(self).stu_selectionAffinity; }
...
- (void)copy:(id __unused)sender {
  ...
  if (string.length != 0) UIPasteboard.generalPasteboard.string = string;
}
...
- (void)selectAll:(id __unused)sender {
  self.selectedTextRange = [self textRangeFromPosition:self.beginningOfDocument
...
- (NSAttributedString*)attributedTextInRange:(UITextRange*)range {
  ...
  return [self.textFrame.truncatedAttributedString attributedSubstringFromRange:
