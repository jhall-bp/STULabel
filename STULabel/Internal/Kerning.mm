...
GlyphForKerningPurposes GlyphForKerningPurposes::find(GlyphSpan glyphSpan,
                                                      ...
                                                      GlyphPositionInSpan position)
{
  ...
  GlyphForKerningPurposes result = {.font = font, .isRightToLeftRun = isRightToLeft};
  ...
  if (STU_UNLIKELY(glyphRange.isEmpty()))
    return result;

  Range<Int> stringRange = run.run().stringRange().intersection(Range{0, string.count()});
  if (STU_UNLIKELY(stringRange.isEmpty()))
    return result;

  result.attributes = attributedString.attributesAtIndex(stringRange.start);
  result.hasDelegate = [result.attributes objectForKey:(__bridge NSAttributedStringKey)kCTRunDelegateAttributeName];
  if (STU_UNLIKELY(result.hasDelegate))
    return result;
  ...
  if (position == rightmostGlyph || (position == lastGlyphInStringOrder && !isRightToLeft)) {
    start = glyphRange.end - 1;
    end = glyphRange.start - 1;
    d = -1;
  } else {
  ...
  for (Int i = start; i != end; i += d) {
    ...
    width += glyphWidth;
    if (glyphWidth <= 0 || width <= 0)
      continue;
    const Int stringIndex = runStringIndices[i];
    if (!stringRange.contains(stringIndex))
      continue;
    ...
    const Float64 unkernedWidth =
        CTFontGetAdvancesForGlyphs(font, kCTFontOrientationHorizontal, &glyph, &unkernedAdvance, 1);
    if (unkernedWidth <= 0)
      continue;
...
STU_NO_INLINE
static CFDictionaryRef createTypesetterOptionsWithEmbeddingLevel(const int embedddingLevel) CF_RETURNS_RETAINED
{
  ...
  return CFDictionaryCreate(nullptr, keys, values, 1, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);
}
                                    ...
                                    const NSStringRef &string1)
{
  ...
  const auto bufferString =
      CFStringCreateWithCharacters(nullptr, reinterpret_cast<const UTF16Char *>(buffer.begin()), bufferLength);

  const auto glyph0String =
      CFStringCreateWithCharacters(nullptr, reinterpret_cast<const UTF16Char *>(&buffer[i0]), glyph0StringLength);
  const auto glyph1String =
      CFStringCreateWithCharacters(nullptr, reinterpret_cast<const UTF16Char *>(&buffer[i1]), glyph1StringLength);
  ...
  const auto attributes0 = CFDictionaryCreateMutableCopy(
      nullptr, sign_cast(glyph0.attributes.count) + 1, (__bridge CFDictionaryRef)glyph0.attributes);

  const auto attributes1 = CFDictionaryCreateMutableCopy(
      nullptr, sign_cast(glyph1.attributes.count) + 1, (__bridge CFDictionaryRef)glyph1.attributes);
  ...
  const auto typesetter =
      CTTypesetterCreateWithAttributedStringAndOptions(attributedString, typesetterOptions(isRightToLeft));
  ...
  do {
    ...
    if (glyphs[0] != glyph0.glyph)
      break;
    if (glyphs[1] != glyph1.glyph)
      break;
    if (!(advances[0].width > 0))
      break;
    if (!(advances[1].width > 0))
      break;
...
HyphenLine
createHyphenLine(const NSAttributedStringRef &originalAttributedString, GlyphRunRef trailingRun, Char32 hyphen)
{
  UTF16Char hyphenChars[2];
  const Int hyphenCharsCount = CFStringGetSurrogatePairForLongCharacter(hyphen, hyphenChars) ? 2 : 1;

  const NSStringRef &string = originalAttributedString.string;
  const auto tg = GlyphForKerningPurposes::find(trailingRun, originalAttributedString, lastGlyphInStringOrder);
  ...
  NoKerning:;
    ...
    const CFStringRef hyphenString = hyphen == hyphenCodePoint
                                         ? hyphenCodePointString
                                         : CFStringCreateWithCharacters(nullptr, hyphenChars, hyphenCharsCount);
    NSAttributedString *hyphenAttributedString;
    if (!tg.hasDelegate) {
      hyphenAttributedString = [[NSAttributedString alloc] initWithString:(__bridge NSString *)hyphenString
    ...
    result.line = CTLineCreateWithAttributedString((__bridge CFAttributedStringRef)hyphenAttributedString);
    result.runIndex = 0;
    result.glyphIndex = -1;
  ...
  { // Construct a CTLine with the trailing glyph plus the hyphen.
    ...
    CFString *const bufferString =
        CFStringCreateWithCharacters(nullptr, reinterpret_cast<const UTF16Char *>(buffer.begin()), buffer.count());
    auto *const attributedString = [[NSMutableAttributedString alloc] initWithString:(__bridge NSString *)bufferString
                                                                          attributes:tg.attributes];
    CFRelease(bufferString);
    const auto glyphString =
        CFStringCreateWithCharacters(nullptr, reinterpret_cast<const UTF16Char *>(buffer.begin()), glyphStringLength);
    ...
    const bool fontHasHyphen = CTFontGetGlyphsForCharacters(tg.font, hyphenChars, hyphenGlyphs, hyphenCharsCount);
    if (!fontHasHyphen) {
      [attributedString
          addAttribute:NSFontAttributeName
                 value:originalAttributedString.attributeAtIndex(NSFontAttributeName, tg.stringRange.start)
    ...
    const CTTypesetterRef ts = CTTypesetterCreateWithAttributedStringAndOptions(
        (__bridge CFAttributedStringRef)attributedString, typesetterOptions(tg.isRightToLeftRun));
    ...
    for (Int i = 0; i < runs.count(); ++i) {
      ...
      for (Int j = 0; j < advances.count(); ++j) {
        ...
        if (nonZeroWidthGlyphCount++ == !tg.isRightToLeftRun) {
          ...
          if (glyph != trailingGlyph)
            goto NoKerning;
          kernedTrailingGlyphAdvance = width;
        }
      }
    }
    if (nonZeroWidthGlyphCount != 2)
      goto NoKerning;
  }
  guard.dismiss();
