                                ...
                                IndexInTruncationToken indexInTruncationToken) const
{
  ...
  const Int32 lineIndex =
      narrow_cast<Int32>(binarySearchFirstIndexWhere(lineStringIndices(),
  ...
  if (indexInOriginalString < para.excisedRangeInOriginalString().start) {
    index = para.rangeInTruncatedString.start + (indexInOriginalString - para.rangeInOriginalString.start);
  } else if (indexInOriginalString >= para.excisedRangeInOriginalString().end &&
             indexInOriginalString < para.rangeInOriginalString.end) {
    index = para.rangeInTruncatedString.end + (indexInOriginalString - para.rangeInOriginalString.end);
  } else {
    if (STU_UNLIKELY(indexInOriginalString >= para.rangeInOriginalString.end)) {
      ...
      if (indexInOriginalString >= p->excisedRangeInOriginalString().end) {
        index = p->rangeInTruncatedString.end + (indexInOriginalString - p->rangeInOriginalString.end);
        goto Return;
      }
    }
    index = para.rangeOfTruncationTokenInTruncatedString().start +
            narrow_cast<Int32>(min(indexInTruncationToken.value, sign_cast(para.truncationTokenLength)));
  }
Return:
  return STUTextFrameIndex{.indexInTruncatedString = sign_cast(index), .lineIndex = sign_cast(lineIndex)};
}
...
Range<TextFrameIndex> TextFrame::range(RangeInOriginalString<NSRange> rangeInOriginalString) const
{
  ...
  const auto end = range.location == rangeEnd
                       ? start
                       : index(IndexInOriginalString{rangeEnd}, IndexInTruncationToken{UInt32{INT32_MAX}});
...
Range<Int32> TextFrame::rangeInOriginalString(STUTextFrameIndex index,
  ...
  if (!line.hasTruncationToken && indexInTruncatedString <= line.rangeInTruncatedString.end) {
    ...
    if (indexInTruncatedString < tokenRange.start) {
      indexInOriginalString =
          para.rangeInOriginalString.start + (indexInTruncatedString - para.rangeInTruncatedString.start);
      ...
      indexInOriginalString = p->rangeInOriginalString.end + (indexInTruncatedString - p->rangeInTruncatedString.end);
    } else {
      ...
      if (outTokenIndex) {
        *outTokenIndex = TruncationTokenIndex{.indexInToken = indexInTruncatedString - tokenRange.start,
...
Range<Int32> TextFrame::rangeInOriginalString(STUTextFrameRange range) const
{
  ...
  const Int32 end = range.end <= range.start ? startRange.end + range.start.isIndexOfInsertedHyphen
