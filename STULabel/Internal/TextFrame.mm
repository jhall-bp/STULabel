...
TextFrame::SizeAndOffset TextFrame::objectSizeAndThisOffset(const TextFrameLayouter &layouter)
{
  ...
  static_assert(IntervalSearchTable::arrayElementSize % alignof(STUTextFrameData) == 0 &&
                ...
                alignof(STUTextFrameData) == alignof(ColorRef) && alignof(STUTextFrameData) >= alignof(TextStyle));
  ...
  const Int stylesTerminatorSize = TextStyle::sizeOfTerminatorWithStringIndex(layouter.rangeInOriginalString().end);
  const ArrayRef<const ColorRef> colors = layouter.colors();

  return {.offset = verticalSearchTableSize + sanitizerGap + lineStringIndicesTableSize + sanitizerGap,
          .size = verticalSearchTableSize + sanitizerGap + lineStringIndicesTableSize + sanitizerGap +
                  sizeof(STUTextFrameData) + layouter.paragraphs().arraySizeInBytes() +
                  layouter.lines().arraySizeInBytes() + sanitizerGap + colors.arraySizeInBytes() + sanitizerGap +
                  layouter.originalStringStyles().dataExcludingTerminator().arraySizeInBytes() +
                  sign_cast(stylesTerminatorSize) + layouter.truncationTokenTextStyleData().arraySizeInBytes() +
                  sanitizerGap};
}

TextFrame::TextFrame(TextFrameLayouter &&layouter, UInt dataSize)
    : STUTextFrameData{.paragraphCount = narrow_cast<Int32>(layouter.paragraphs().count()),
                       ...
                       ._dataSize = dataSize}
{
  ...
  const Int originalStylesTerminatorSize = TextStyle ::sizeOfTerminatorWithStringIndex(stringRange.end);
  const UInt originalStringTextStyleDataSize =
      sign_cast(layouter.originalStringStyles().dataExcludingTerminator().count() + originalStylesTerminatorSize);
  _textStylesData = reinterpret_cast<const uint8_t *>(this) + dataSize - sanitizerGap -
                    layouter.truncationTokenTextStyleData().count() - originalStringTextStyleDataSize;
  ...
  { // Write out the data into the embedded arrays.
    ...
    TextStyle::writeTerminatorWithStringIndex(
        stringRange.end, p - originalStyles.lastStyleSizeInBytes(), ArrayRef{p, originalStylesTerminatorSize});
  ...
  for (TextFrameParagraph &para : paragraphs) {
    ...
    if (isIndented) {
      ...
      if (p.initialExtraLeftIndent == 0) {
        ...
        if (p.initialExtraLeftIndent > 0) {
          ...
          para.nonInitialLinesLeftIndent = p.commonLeftIndent;
          para.initialLinesLeftIndent = p.commonLeftIndent + textScaleFactor * p.initialExtraLeftIndent;

        } else {
          nonInitialLeftIndent -= p.initialExtraLeftIndent;
          para.initialLinesLeftIndent = p.commonLeftIndent;
          para.nonInitialLinesLeftIndent = p.commonLeftIndent - textScaleFactor * p.initialExtraLeftIndent;
        }
      }
      if (p.initialExtraRightIndent == 0) {
        ...
        if (p.initialExtraRightIndent > 0) {
          ...
          para.nonInitialLinesRightIndent = p.commonRightIndent;
          para.initialLinesRightIndent = p.commonRightIndent + textScaleFactor * p.initialExtraRightIndent;

        } else {
          nonInitialRightIndent -= p.initialExtraRightIndent;
          para.initialLinesRightIndent = p.commonRightIndent;
          para.nonInitialLinesRightIndent = p.commonRightIndent - textScaleFactor * p.initialExtraRightIndent;
        }
      }
    }
    ...
    for (; lineIndex < para.lineIndexRange().end; ++lineIndex) {
      ...
      if (isIndented) {
        ...
        const Float64 leftIndent = lineIndex < para.initialLinesEndIndex ? initialLeftIndent : nonInitialLeftIndent;
        const Float64 rightIndent = lineIndex < para.initialLinesEndIndex ? initialRightIndent : nonInitialRightIndent;
  ...
  const Float32 firstLineMinBaselineDistance = layouter.originalStringParagraphs()[0].minBaselineDistance;
  ...
  this->firstLineHeightAboveBaseline =
      scale32 * (firstLine._heightAboveBaseline + max(0.f, (firstLineMinBaselineDistance - firstLineHeight) / 2));

  this->lastLineHeight = scale32 * max(lastLineHeight, lastLineMinBaselineDistance);
  this->lastLineHeightBelowBaselineWithoutSpacing = scale32 * lastLine._heightBelowBaselineWithoutSpacing;
  ...
  this->lastLineHeightBelowBaseline =
      scale32 * (lastLine._heightBelowBaseline + max(0.f, (lastLineMinBaselineDistance - lastLineHeight) / 2));

  STUTextFrameConsistentAlignment consistentAlignment = stuTextFrameConsistentAlignment(paragraphs[0].alignment);
  ...
  bool hasMaxTypographicWidth = consistentAlignment != STUTextFrameConsistentAlignmentNone && !isTruncated && !isScaled;
  if (hasMaxTypographicWidth) {
    Int32 i = 0;
    for (const TextFrameParagraph &para : paragraphs) {
      if (para.lineIndexRange().end == ++i)
        continue;
  ...
  this->flags = static_cast<STUTextFrameFlags>(
      static_cast<STUTextFrameFlags>(flags) | (isTruncated ? STUTextFrameIsTruncated : 0) |
      (isScaled ? STUTextFrameIsScaled : 0) | (hasMaxTypographicWidth ? STUTextFrameHasMaxTypographicWidth : 0));
}
