...
PurgeableImage::PurgeableImage(CGSize size,
                               CGFloat scale,
    ...
    : PurgeableImage{SizeInPixels<UInt32>{size, scale}, scale, backgroundColor, format, formatOptions, drawingFunction}
{}

PurgeableImage::PurgeableImage(SizeInPixels<UInt32> size,
                               CGFloat scale,
                               __nullable CGColorRef backgroundColor,
                               STUPredefinedCGImageFormat format,
                               STUCGImageFormatOptions formatOptions,
    ...
    : PurgeableImage{}
{
  ...
  if (__builtin_mul_overflow(size.width, imageFormat.bitsPerPixel / 8, &bytesPerRow))
    goto Failure;
  if (bytesPerRow > stu::maxValue<UInt> - 31)
    goto Failure;
  bytesPerRow = roundUpToMultipleOf<32>(bytesPerRow);
  if (bytesPerRow / 32 > stu::maxValue<UInt32>)
    goto Failure;

  if (__builtin_mul_overflow(bytesPerRow, size.height, &allocationSize))
    goto Failure;
