...
#define addPropertyObserver(name, opts) [layer addObserver:observer forKeyPath:name options:opts context:nil];
...
#undef addPropertyObserver
  ...
  objc_setAssociatedObject(layer, &visibleBoundsObserver, observer, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
{
  auto const observer =
      static_cast<STULabelSuperlayerObserver *>(objc_getAssociatedObject(superlayer, &visibleBoundsObserver));
...
#undef removeObserver
  ...
  objc_setAssociatedObject(superlayer, &visibleBoundsObserver, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
{
  CALayer *__unsafe_unretained sublayer = superlayers_.isEmpty() ? layer_ : superlayers_[$ - 1].layer().unretained;
...
void LayerVisibleBoundsObserver ::_private_superlayerIsBeingRemovedOrDestroyed(CALayer *__unsafe_unretained superlayer)
{
  const auto optIndex = superlayers_.indexWhere([&](SuperlayerRef sr) { return sr.layer() == superlayer; });
...
void LayerVisibleBoundsObserver ::_private_superlayerMasksToBoundsChanged(CALayer *__unsafe_unretained superlayer,
                                                                          bool masksToBounds)
{
...
{
  if (visibleBoundsMayHaveChanged_)
    return;
