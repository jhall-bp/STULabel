...
struct STUTextBackgroundSegment
{
  ...
  const STUTextBackgroundSegment *next() const
  {
    ...
    return isLast ? nullptr : reinterpret_cast<const STUTextBackgroundSegment *>(spans + spanCount);
  }
...
public:
  TempBackgroundSegments(const TextFrame &textFrame, Range<Int> lineRange, Optional<TextStyleOverride &> styleOverride);

  const BackgroundSegment *first() const
  {
    return data_.isEmpty() ? nullptr : reinterpret_cast<const BackgroundSegment *>(&data_[0]);
  }

  Int dataSize() const { return data_.count(); }
};

TaggedRangeLineSpans TempBackgroundSegments::findBackgroundLineSpans(const ArrayRef<const TextFrameLine> lines,
                                                                     Optional<TextStyleOverride &> styleOverride)
{
  return findAndSortTaggedRangeLineSpans(
      lines,
      styleOverride,
      ...
      data_{freeCapacityInCurrentThreadLocalAllocatorBuffer}
{
  ...
  if (data_.isEmpty()) {
    BackgroundSegment *const bs =
        reinterpret_cast<BackgroundSegment *>(data_.append(repeat(uninitialized, sizeof(BackgroundSegment))));
    *bs = BackgroundSegment{.isLast = true};
  }
  data_.trimFreeCapacity();
}

void TempBackgroundSegments::appendSegment(const TextStyle::BackgroundInfo &info,
                                           const STUBackgroundAttribute *__nullable __unsafe_unretained attrib,
                                           ...
                                           ArrayRef<const TextFrameLine> lines)
{
  ...
  *p = BackgroundSegment{.spanCount = static_cast<UInt32>(inputSpans.count()),
  ...
  if (attrib) {
    ...
    if (leftInset != 0 || rightInset != 0) {
      ...
      const Int newCount = adjustTextLineSpansByHorizontalInsetsAndReturnNewCount(
          ArrayRef{const_cast<TextLineSpan *>(spans.begin()), spans.count()}, HorizontalInsets{leftInset, rightInset});
      removeLastSpans(spans.count() - newCount);
    }
  }
  if (firstLineIndex != lastLineIndex && (!attrib || attrib->_extendTextLinesToCommonHorizontalBounds)) {
  ...
  const auto verticalInsets = !attrib ? VerticalEdgeInsets{} : VerticalEdgeInsets(attrib->_edgeInsets);
...
void BackgroundSegment::draw(ArrayRef<const TextFrameLine> lines, DrawingContext &context) const
{
  if (spanCount == 0)
    return;
  const Range<Int> lineRange = sign_cast(lineIndexOffset) + Range{0, spans[spanCount - 1].lineIndex + 1};

  TempArray<TextLineVerticalPosition> verticalPositions = textLineVerticalPositions(
      lines[lineRange],
      context.displayScale(),
      !attribute ? VerticalEdgeInsets{} : VerticalEdgeInsets(attribute->_edgeInsets),
      VerticalOffsets{.textFrameOriginY = context.textFrameOrigin().y, .ctmYOffset = context.ctmYOffset()});
  ...
  const CGPathDrawingMode mode = !strokeColorIndex ? kCGPathFill : !colorIndex ? kCGPathStroke : kCGPathFillStroke;
  if (strokeColorIndex) {
  ...
  addLineSpansPath(*path,
                   ArrayRef{spans, sign_cast(spanCount)},
                   verticalPositions,
                                   ...
                                   DrawingContext &context)
{
  ...
  for (; bs; bs = bs->next()) {
    if (!clipRect.overlaps(bs->bounds))
      continue;
    if (context.isCancelled())
      return;
...
void TextFrame::drawBackground(Range<Int> clipLineRange, DrawingContext &context) const
{
  ...
  while (clipLineRange.start > 0 && (lines[clipLineRange.start - 1].textFlags() & TextFlags::hasBackground)) {
    clipLineRange.start -= 1;
  }
  while (clipLineRange.end < lines.count() && (lines[clipLineRange.end].textFlags() & TextFlags::hasBackground)) {
    clipLineRange.end += 1;
  }

  const Optional<TextStyleOverride &> styleOverride = context.styleOverride();
  if (styleOverride &&
      (!(styleOverride->flagsMask & TextFlags::hasBackground) || !styleOverride->drawnRange.contains(range()))) {
  ...
  const BackgroundSegment *bs = atomic_load_explicit(frameBackgroundSegments, memory_order_relaxed);
  if (bs) {
    ...
    if (!atomic_compare_exchange_strong_explicit(
            frameBackgroundSegments, &expected, bs, memory_order_release, memory_order_acquire)) {
