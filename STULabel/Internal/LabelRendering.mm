                                                  ...
                                                  const STUCancellationFlag *__nullable cancellationFlag)
{
  ...
  const bool useImageBounds =
      !params.clipsContentToBounds ||
      (mode != LabelRenderMode::drawInCAContext &&
       info.minFrameSize.area() <= (2. / 3) * params.size().width * params.size().height &&
       params.displayScale() * (1 + max(info.minFrameSize.width, info.minFrameSize.height)) <= maxImagePixelDimension);

  Rect<CGFloat> bounds{uninitialized};
  Rect<CGFloat> imageBounds{uninitialized};
  bool mayBeClipped = true;
  if (useImageBounds) {
    imageBounds = STUTextFrameGetImageBoundsForRange(textFrame,
                                                     STUTextFrameGetRange(textFrame),
                                                     CGPoint{},
                                                     params.displayScale(),
                                                     params.drawingOptions,
                                                     cancellationFlag);
    const CGFloat tolerance = params.displayScale().inverseValue() / 4;
    bounds = renderBoundsForTextFrameImageBounds(
        imageBounds, info, params.size(), params.edgeInsets(), tolerance, Out{mayBeClipped});
  ...
  bool isGrayscale =
      mode != LabelRenderMode::drawInCAContext && !params.neverUseGrayscaleBitmapFormat &&
      !(frameFlags & STUTextFrameMayNotBeGrayscale) &&
      (!params.drawingBlock || (params.drawingBlockColorOptions & STULabelDrawingBlockOnlyUsesTextColors));

  bool shouldDrawBackgroundColor =
      (params.backgroundColorFlags() & ColorFlags::isOpaque) &&
      (mode == LabelRenderMode::drawInCAContext || (mode == LabelRenderMode::image && isGrayscale));
  ...
  const bool useExtendedColor =
      ...
      ((frameFlags & STUTextUsesExtendedColor) ||
       ((params.drawingBlockColorOptions & STULabelDrawingBlockUsesExtendedColors) && params.drawingBlock));

  if (!useExtendedColor && (params.backgroundColorFlags() & ColorFlags::isExtended)) {
    shouldDrawBackgroundColor = false;
  }

  if (shouldDrawBackgroundColor && isGrayscale && (params.backgroundColorFlags() & ColorFlags::isNotGray)) {
  ...
  const bool isOpaque = shouldDrawBackgroundColor && (params.backgroundColorFlags() & ColorFlags::isOpaque);
...
void drawLabelTextFrame(const STUTextFrame *__unsafe_unretained textFrame,
                        ...
                        const STUCancellationFlag *__nullable cancellationFlag)
{
  ...
  if (!drawingBlock) {
    drawTextFrame(
        textFrame, range, textFrameOrigin, context, contextBaseCTM_d, pixelAlignBaselines, options, cancellationFlag);
  } else {
    STU_DEBUG_ASSERT(!options || options->impl.isFrozen());
    STULabelDrawingBlockParameters *const p = createLabelDrawingBlockParametersInstance(
        // Pointer to non-const is an Obj-C convention.
        const_cast<STUTextFrame *>(textFrame),
        range,
        textFrameOrigin,
        context,
        contextBaseCTM_d,
        pixelAlignBaselines,
        const_cast<STUTextFrameDrawingOptions *>(options),
        cancellationFlag);
