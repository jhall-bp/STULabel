...
GlyphsWithPositions GlyphSpan::getGlyphsWithPositionsImpl(GlyphRunRef run, CFRange glyphRange)
{
  ...
  const Int bufferSize = count * sign_cast((positions ? 0 : sizeof(CGPoint)) + (glyphs ? 0 : sizeof(CGGlyph)));
...
STU_NO_INLINE
Rect<CGFloat> GlyphSpan::imageBoundsImpl(GlyphRunRef run, CFRange glyphRange, LocalGlyphBoundsCache &glyphBoundsCache)
{
...
{
  if (STU_UNLIKELY(glyphRange.length <= 0))
    return {};
...
STU_NO_INLINE
bool GlyphSpan::copyInnerCaretOffsetsForLigatureGlyphAtIndexImpl(GlyphRunRef run,
                                                                 Int glyphIndex,
                                                                 ArrayRef<CGFloat> outInnerCaretOffsets)
{
  ...
  Int n = CTFontGetLigatureCaretPositions(font, glyph, outInnerCaretOffsets.begin(), outInnerCaretOffsets.count());
  if (n == outInnerCaretOffsets.count())
    return true;
  if (n != 0)
    return false;
...
static bool checkCanUseFastCTRunFontGetter(CTRun *__nonnull run)
{
  ...
  {
    auto *const s = [[NSAttributedString alloc] initWithString:@"x" attributes:@{NSFontAttributeName : font1}];
    if (!checkCTRunsAreCompatibleWithFastCTRunFontGetter(s))
      return false;
  }
  {
    auto *const s = [[NSAttributedString alloc] initWithString:@"😀" attributes:@{NSFontAttributeName : font1}];
    if (!checkCTRunsAreCompatibleWithFastCTRunFontGetter(s))
      return false;
  }
  ...
  UIFont *const font2 = [UIFont fontWithName:@"HelveticaNeue" size:16];
  {
    ...
    [s appendAttributedString:[[NSAttributedString alloc] initWithString:@"x "
                                                              attributes:@{NSFontAttributeName : font2}]];
    [s appendAttributedString:[[NSAttributedString alloc] initWithString:@"是"
                                                              attributes:@{NSFontAttributeName : font2}]];
    if (!checkCTRunsAreCompatibleWithFastCTRunFontGetter(s))
      return false;
  }
  {
    auto *const s = [[NSMutableAttributedString alloc] init];
    [s appendAttributedString:[[NSAttributedString alloc] initWithString:@"0123456789"
                                                              attributes:@{NSFontAttributeName : font2}]];
    [s appendAttributedString:[[NSAttributedString alloc]
                                  initWithString:@"abcdef"
                                      attributes:@{NSFontAttributeName : font2, @"stuTest" : NSNull.null}]];
    if (!checkCTRunsAreCompatibleWithFastCTRunFontGetter(s))
      return false;
  }
  {
    STUTextAttachment *const attachment = [[STUTextAttachment alloc] initWithWidth:20
        ascent:10
        descent:10
        leading:0
        imageBounds:CGRect {}
...
CTFont *GlyphRunRef::getFont(CTRun *run)
{
  ...
  if (sizeof(void *) == 8) {
    ...
    if (STU_UNLIKELY(oldStatus <= Status::untested)) {
      if (status == Status::mustUseSlowpath)
        goto SlowPath;
