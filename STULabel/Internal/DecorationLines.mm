...
STU_INLINE
static void
roundOffsetAndThickness(InOut<CGFloat> inOutOffset, InOut<CGFloat> inOutThickness, const DisplayScale &displayScale)
{
...
static UnderlineOffsetAndThickness
calculateUnderlineOffsetAndThickness(CGFloat minY,
                                     CGFloat thickness,
                                     ...
                                     const Optional<DisplayScale> &originalDisplayScale)
{
  ...
  if (displayScale.storage().displayScaleOrZero() != originalDisplayScale.storage().displayScaleOrZero() &&
      originalDisplayScale) {
...
static void
findXBoundsOfIntersectionsOfGlyphsWithHorizontalLine(CGFloat runXOffset,
                                                     GlyphSpan span,
                                                     CGFloat minY,
                                                     CGFloat maxY,
                                                     CGFloat dilation,
                                                     ...
                                                     Optional<SortedIntervalBuffer<CGFloat> &> upperStripeBuffer)
{
  ...
  for (Int i = 0; i < gwp.count(); ++i) {
    ...
    if (!bounds.y.overlaps(Range{minY, maxY}))
      continue;
...
static void
removeLinePartsNotIntersectingClipRectAndMergeIdenticallyStyledAdjacentUnderlines(TempVector<DecorationLine> &buffer,
                                                                                  const Rect<CGFloat> &clipRect)
{
  DecorationLine *previous = nullptr;
  DecorationLine *end = nullptr;
  for (DecorationLine &u : buffer) {
    const auto rect = u.rectLLO();
    bool remove = !rect.overlaps(clipRect) && !rectShadowOverlapsRectLLO(rect, u.shadowInfo, clipRect);
    if (previous) {
      if (!remove) {
        if (u.isUnderlineContinuation && previous->colorIndex == u.colorIndex &&
            (previous->shadowInfo == u.shadowInfo ||
             (previous->shadowInfo && u.shadowInfo && *previous->shadowInfo == *u.shadowInfo))) {
...
Underlines Underlines::find(const TextFrameLine &line, DrawingContext &context)
{
  ...
  TempArray<DecorationLine> lines;
  {
    ...
    line.forEachStyledGlyphSpan(
        ...
        [&](const StyledGlyphSpan &span, const TextStyle &style, const Range<Float64> x_f64) -> ShouldStop {
          ...
          if (!x.isEmpty() && font) {
            ...
            CGFloat minY;
            CGFloat thickness;
            if (previous && previousTextStyle == &style && !style.isOverrideStyle() && previousFont == font) {
              minY = previous->offsetLLO;
              thickness = previous->thickness;
            } else {
              if (!style.isOverrideStyle() || (!(context.styleOverride()->flags & TextFlags::hasUnderline))) {
            ...
            const bool isContinuation = previous && previous->x.end == x.start && previous->style == lineStyle;
            ...
            buffer.append(
                DecorationLine{.x = x,
                               .fullLineXStart = fullLineXStart,
                               .offsetLLO = minY,
                               .thickness = thickness,
                               .style = lineStyle,
                               .colorIndex = info.colorIndex ? *info.colorIndex : context.textColorIndex(style),
    ...
    if (!context.isCancelled()) {
      ...
      for (DecorationLine &u : buffer.reversed()) {
        ...
        if (!u.hasUnderlineContinuation) {
          ...
          const auto ot = calculateUnderlineOffsetAndThickness(
              u.offsetLLO, u.thickness, u.style, context.displayScale(), context.textFrameDisplayScale());
          offset = ot.offsetLLO;
  ...
  if (lines.count() != 0 && !context.isCancelled()) {
    ...
    line.forEachStyledGlyphSpan(
        TextFlags::hasUnderline,
        context.styleOverride(),
        [&](const StyledGlyphSpan &span, const TextStyle &__unused style, Range<Float64> x_f64) -> ShouldStop {
          ...
          findXBoundsOfIntersectionsOfGlyphsWithHorizontalLine(
              span.ctLineXOffset,
              span.glyphSpan,
              u.originalOffsetLLO - u.originalThickness / 2,
              u.originalOffsetLLO + u.originalThickness / 2,
              dilation,
              context.displayScale(),
              context.glyphBoundsCache(),
              buffer,
              isDoubleLine ? Optional<SortedIntervalBuffer<CGFloat> &>(buffer2) : none);
                                         ...
                                         LocalFontInfoCache &fontInfoCache)
{
  ...
  const auto enlargeYBoundsForPreviousUnderline = [&] {
    if (previousStyle) { // clang analyzer bug or false positive
      const Range<CGFloat> y = calculateUnderlineOffsetAndThickness(
                                   previousMinY, previousThickness, previousUnderlineStyle, displayScale, displayScale)
                                   .yLLO();
  ...
  line.forEachStyledGlyphSpan(TextFlags::hasUnderline,
                              styleOverride,
                              [&](const StyledGlyphSpan &span, const TextStyle &style, const Range<Float64> x) {
                                ...
                                if (const auto shadowInfo = style.shadowInfo(); STU_UNLIKELY(shadowInfo)) {
                                  bounds.x =
                                      bounds.x.convexHull((x + shadowInfo->offsetX).outsetBy(shadowInfo->blurRadius));
...
Strikethroughs Strikethroughs::find(const TextFrameLine &line, DrawingContext &context)
{
  ...
  line.forEachStyledGlyphSpan(
      ...
      [&](const StyledGlyphSpan &span, const TextStyle &style, Range<Float64> x_f64) -> ShouldStop {
        ...
        if (previous && previous->x.end == x.start && &style == previousTextStyle && !style.isOverrideStyle() &&
            font == previousFont) {
        ...
        const NSUnderlineStyle lineStyle = static_cast<NSUnderlineStyle>(info.style);
        const auto ot =
            OffsetAndThickness::forStrikethrough(span, style, font, context.displayScale(), context.fontInfoCache());
        const ColorIndex colorIndex = info.colorIndex ? *info.colorIndex : context.textColorIndex(style);
        if (previous && previous->x.end == x.start && previous->offsetLLO == ot.offsetLLO &&
            previous->unroundedThickness == ot.unroundedThickness && previous->style == lineStyle &&
          ...
          buffer.append(DecorationLine{.x = x,
                                       .fullLineXStart = x.start,
                                       .offsetLLO = ot.offsetLLO,
                                       .thickness = ot.thickness,
                                       .unroundedThickness = ot.unroundedThickness,
...
struct DrawShadow : Parameter<DrawShadow>
{
  using Parameter::Parameter;
};

static void drawDecorationLine(const DecorationLine &line,
                               Range<CGFloat> x,
                               DoubleLineStripe stripes,
                               ...
                               DrawingContext &context)
{
  ...
  if (x.end - x.start < originalThickness)
    return;
...
STU_INLINE
void drawDecorationLines(ArrayRef<const DecorationLine> lines, DrawShadow drawShadow, DrawingContext &context)
{
                                        ...
                                        DrawingContext &context)
{
  ...
  for (const DecorationLine &line : lines) {
    if ((drawShadow && !line.shadowInfo) ||
        (stripe == DoubleLineStripe::upper && (line.style & NSUnderlineStyleDouble) != NSUnderlineStyleDouble)) {
      continue;
    }
...
void Underlines::drawLLO(DrawingContext &context) const
{
  if (hasShadow) {
    ...
    if (hasDoubleLine) {
      drawDecorationLinesWithGaps(lines, DoubleLineStripe::upper, upperLinesGaps, DrawShadow{true}, context);
      if (context.isCancelled())
        return;
    }
    drawDecorationLinesWithGaps(lines, DoubleLineStripe::lower, lowerLinesGaps, DrawShadow{true}, context);
    if (context.isCancelled())
      return;
  }
  if (hasDoubleLine) {
    drawDecorationLinesWithGaps(lines, DoubleLineStripe::upper, upperLinesGaps, DrawShadow{false}, context);
    if (context.isCancelled())
      return;
  }
  drawDecorationLinesWithGaps(lines, DoubleLineStripe::lower, lowerLinesGaps, DrawShadow{false}, context);
}
