...
STU_NO_INLINE
NSStringRef::NSStringRef(CFString *string, Optional<Ref<TempStringBuffer>> optBuffer) : string_(string)
{
    ...
    if (noBuffer) {
      if (optBuffer && length < 2048) {
        ...
        bufferOrMethod_.method = reinterpret_cast<GetCharactersMethod>(
            [(__bridge NSString *)string methodForSelector:@selector(getCharacters:range:)]);
...
{
  UTF16Char result;
  bufferOrMethod_.method(
      (__bridge NSString *)string_, @selector(getCharacters:range:), &result, NSRange{sign_cast(index), 1});
  return result;
}
...
Char32 NSStringRef::codePointAtUTF16Index_slowPath(Int index) const
{
  ...
  bufferOrMethod_.method(
      (__bridge NSString *)string_, @selector(getCharacters:range:), chars, NSRange(Range{index, Count{n}}));
...
void NSStringRef::copyUTF16Chars_slowPath(NSRange utf16IndexRange, Char16 *out) const
{
  ...
  bufferOrMethod_.method((__bridge NSString *)string_,
                         @selector(getCharacters:range:),
                         reinterpret_cast<UTF16Char *>(out),
                         utf16IndexRange);
}

Int NSStringRef::copyRangesOfGraphemeClustersSkippingTrailingIgnorables(Range<Int> stringRange,
                                                                        ArrayRef<Range<Int>> outStringRanges) const
{
  Int count = 0;
...
template <NSStringRefBufferKind kind> class NSStringRefBuffer
{
  static_assert(kind == NSStringRefBufferKind::utf16 || kind == NSStringRefBufferKind::ascii);
...
public:
  ...
  NSStringRefBuffer(const NSStringRef &string, Range<Int> range, Int startIndex, Storage __unused)
      : buffer_{static_cast<const Char *>(string.bufferOrMethod_.buffer)}, start_{range.start}, end_{range.end},
        index_{startIndex}
  {
...
template <> class NSStringRefBuffer<NSStringRefBufferKind::none>
{
public:
  ...
  NSStringRefBuffer(const NSStringRef &string, Range<Int> range, Int startIndex, Storage &storage)
      : string_{(__bridge NSString *)string.string_}, getCharacters_{string.bufferOrMethod_.method},
        array_{storage.array}, startIndex_{range.start}, endIndex_{range.end}, arrayStartIndex_{startIndex},
        arrayCount_{0}, indexInArray_{0}
  {
    STU_DEBUG_ASSERT(range.start <= startIndex && startIndex <= range.end);
  }
  ...
  Optional<Char16> getUTF16CharAndSkipToNext()
  {
    if (STU_UNLIKELY(indexInArray_ == arrayCount_)) {
      ...
      arrayCount_ = n;
      getCharacters_(
          string_, @selector(getCharacters:range:), array_, NSRange{sign_cast(arrayStartIndex_), sign_cast(n)});
    }
...
STU_INLINE Int indexOfFirstUTF16CharWhereImpl(const NSStringRef &string, const Range<Int> range, Predicate &&predicate)
{
  ...
  for (;;) {
    ...
    if (!optCh || predicate(index, *optCh))
      break;
  }
  return index;
}
...
STU_NO_INLINE
Int NSStringRef ::indexOfFirstUTF16CharWhereImpl(Range<Int> range, FunctionRef<bool(Int index, Char16)> predicate) const
{
...
STU_NO_INLINE
Int NSStringRef ::indexOfFirstCodePointWhereImpl(Range<Int> range, FunctionRef<bool(Int index, Char32)> predicate) const
{
  const BufferKind kind = kind_;
  if (kind == BufferKind::utf16) {
...
STU_NO_INLINE
Int NSStringRef ::indexOfEndOfLastCodePointWhereImpl(Range<Int> range,
...
static constexpr CategoryPairCase categoryPairCase_init(Category a, Category b)
{
  ...
  if (a == C::controlCR && b == C::controlLF)
    return Case::noBreak;
  if (a == C::controlCR || a == C::controlLF || a == C::controlOther)
    return Case::isBreak;
  switch (b) {
  case C::other:
    break;
  ...
  case C::hangulLVT:
    ...
    break;
  case C::hangulV:
    if (a == C::hangulL || a == C::hangulV || a == C::hangulLV)
      return Case::noBreak;
    break;
  case C::hangulT:
  ...
  case C::extend:
    if (a == C::extend)
      return Case::extend_extend;
    if (a == C::extendedPictographic)
      return Case::picto_extend;
    return Case::noBreak;
  case C::zwj:
    if (a == C::extend)
      return Case::extend_zwj;
    if (a == C::extendedPictographic)
      return Case::picto_zwj;
    return Case::noBreak;
  case C::spacingMark:
    return Case::noBreak;
  case C::extendedPictographic:
    if (a == C::zwj)
      return Case::zwj_picto;
    break;
  case C::regionalIndicator:
    if (a == C::regionalIndicator)
      return Case::ri_ri;
    if (a == C::prepend)
      return Case::prepend_ri;
  }
  if (a == C::prepend)
    return Case::noBreak;
  return Case::isBreak;
...
constexpr int categoryPairCaseMatrixLength = categoryPairCaseMatrixColumnCount * graphemeClusterCategoryCount;
...
constexpr CategoryPairCaseMatrix createCategoryPairCaseMatrix()
{
  ...
  for (int i = 0; i < categoryCount; ++i) {
    for (int j = 0; j < columnCount; ++j) {
      array[i * columnCount + j] =
          j >= categoryCount ? CategoryPairCase::noBreak : categoryPairCase_init(Category(j), Category(i));
    }
  }
  return array;
}
...
struct BreakFinder
{
  ...
  explicit BreakFinder(const NSStringRef &string) : string{string}, startIndex{-1} {}
  ...
  bool advanceForwards(Category category)
  {
    ...
    case Case::extend_extend:
      newState = state == State::picto_extend || state == State::maybe_picto_extend ? state : State::other;
      break;
    ...
    case Case::ri_ri:
      if (state == State::ri_ri ||
          (state == State::maybe_ri_ri && numberOfConsecutiveRegionalIndicatorsBefore(string, startIndex) % 2 != 0)) {
        return false;
      }
  ...
  bool advanceBackwards(Int index, Category category)
  {
    ...
    case Case::zwj_picto:
      ...
      return false;
    case Case::ri_ri:
      if (state == State::ri_ri || numberOfConsecutiveRegionalIndicatorsBefore(string, index) % 2 != 0) {
        return false;
      }
      newState = State::ri_ri;
      break;
  ...
  static bool hasPictoPrefix(const NSStringRef &string, Int index, Category category)
  {
    ...
    string.indexOfEndOfLastCodePointWhere(Range{0, index}, [&](Char32 cp) -> bool {
      ...
      hasPrefix = c0 == Category::extendedPictographic && (c1 == Category::extend || c1 == Category::zwj);
      return true;
    });
    return hasPrefix;
  }
  ...
  STU_NO_INLINE
  static Int startIndexOfPictoZWJSequence(const NSStringRef &string, const Int indexOfZWJBeforePicto)
  {
    ...
    string.indexOfEndOfLastCodePointWhere(Range{0, indexOfZWJBeforePicto}, [&](Int index, Char32 cp) -> bool {
      const Category c0 = graphemeClusterCategory(cp);
      const Category c1 = category;
      category = c0;
      switch (c0) {
      case Category::zwj:
        if (c1 == Category::extendedPictographic)
          return false;
        return true;
      case Category::extend:
...
STU_NO_INLINE
Int NSStringRef::indexOfFirstGraphemeClusterBreak_noBuffer(const bool greaterThan, const Int index) const
{
  ...
  return detail::indexOfFirstCodePointWhereImpl<BufferKind::none>(
      *this,
      Range{index0, count},
      [index, isFirst = true, finder = BreakFinder(*this)](Int i, Char32 cp) STU_INLINE_LAMBDA mutable -> bool {
        if (!isFirst) {
...
STU_NO_INLINE
Int NSStringRef::indexOfLastGraphemeClusterBreakImpl_noBuffer(const bool lessThan, const Int index) const
{
  STU_DEBUG_ASSERT(kind_ == BufferKind::none);
  const Int count = this->count();
  if (lessThan) {
  ...
  const Int endIndex = detail::indexOfEndOfLastCodePointWhereImpl<BufferKind::none>(
      *this, Range{0, min(index + (lessThan ? 0 : 2), count)}, [&](Int i, Char32 cp) STU_INLINE_LAMBDA -> bool {
        if (!isFirst) {
...
STU_NO_INLINE
Int NSStringRef ::endIndexOfGraphemeClusterAtImpl_utf16Buffer(Int index,
                                                              GraphemeClusterCategory category,
                                                              Int nextIndex) const
{
  using namespace grapheme_cluster;
  ...
  do {
    ...
    if (!finder.advanceForwards(category))
      break;
    index = nextIndex;
  } while (index != count);
  return index;
...
STU_NO_INLINE
Int NSStringRef ::indexOfLastGraphemeClusterBreakBeforeImpl_utf16Buffer(Int index,
