...
static NSAttributedString *__nonnull creatTextFrameTruncatedAttributedString(const TextFrame &textFrame)
    NS_RETURNS_RETAINED
{
  ...
  if (textFrame.rangeInOriginalStringIsFullString && rangeInOriginalString == textFrame.rangeInOriginalString()) {
    mutableString = [textFrame.originalAttributedString mutableCopy];
  } else {
    NSAttributedString *const substring =
        [textFrame.originalAttributedString attributedSubstringFromRange:NSRange(rangeInOriginalString)];
...
Unretained<NSAttributedString * __nonnull> TextFrame::truncatedAttributedString() const
{
  ...
  if (atomic_load_explicit(pAttributedString, memory_order_relaxed)) {
    return (__bridge NSAttributedString *)atomic_load_explicit(pAttributedString, memory_order_acquire);
  }
  CFAttributedStringRef expected = nil;
  CFAttributedStringRef retained =
      (__bridge_retained CFAttributedStringRef)creatTextFrameTruncatedAttributedString(*this);
  if (atomic_compare_exchange_strong_explicit(
          pAttributedString, &expected, retained, memory_order_release, memory_order_acquire)) {
