...
ShouldStop TextFrameLine::forEachCTLineSegment(
    ...
    FunctionRef<ShouldStop(TextLinePart, CTLineXOffset, CTLine &, Optional<GlyphSpan>)> body) const
{
  ...
  if (ctLine) {
    if (!shouldIterNonTokenRunsIndividually && _leftPartEnd.runIndex < 0) {
      ...
      if (endGlyphIndex > 0) {
        const auto shouldStop = body(TextLinePart::originalString,
                                     CTLineXOffset{0},
                                     *ctLine,
  ...
  if (hyphenRunIndex < 0 && !((tokenTextFlags() | everyRunFlag) & mask.flags)) {
    const auto shouldStop = body(TextLinePart::truncationToken, CTLineXOffset{this->leftPartWidth}, *tokenCTLine, none);
    if (shouldStop)
      return shouldStop;
  } else {
    ...
    if (hyphenRunIndex < 0) {
      for (CTRun *const run : tokenRuns) {
        const auto shouldStop =
            body(TextLinePart::truncationToken, CTLineXOffset{this->leftPartWidth}, *tokenCTLine, run);
        if (shouldStop)
          return shouldStop;
      }
    } else if (hyphenRunIndex < tokenRuns.count()) {
      const GlyphRunRef run{tokenRuns[hyphenRunIndex]};
      const GlyphSpan span =
          _hyphenGlyphIndex < 0 ? GlyphSpan{run} : GlyphSpan{run, {_hyphenGlyphIndex, Count{1}}, unchecked};
      const auto shouldStop =
          body(TextLinePart::insertedHyphen, CTLineXOffset{this->leftPartWidth - _hyphenXOffset}, *tokenCTLine, span);
      if (shouldStop)
        return shouldStop;
    }
  }
  ...
  if (startGlyphIndex > 0) {
    const auto shouldStop = body(
        TextLinePart::originalString, rightPartXOffset, *ctLine, GlyphSpan{runs[startRunIndex], {startGlyphIndex, $}});
    if (shouldStop)
      return shouldStop;
    startRunIndex += 1;
  }
...
struct TokenStringOffset : Parameter<TokenStringOffset, Int32>
{
...
template <> class stu::OptionalValueStorage<stu_label::TokenStringOffset>
{
...
STU_NO_INLINE static ShouldStop
forEachStyledGlyphSpanSubspan(StyledGlyphSpan &,
                              Float64 minX,
                              Float64 maxX,
                              const TextStyle &,
                              TextStyleOverride &,
...
[[nodiscard]] STU_INLINE ShouldStop
forEachStyledGlyphSpanSubspan(StyledGlyphSpan &span,
                              Float64 minX,
                              Float64 maxX,
                              ...
                              FunctionRef<ShouldStop(const StyledGlyphSpan &, const TextStyle &, Range<Float64>)> body)
{
  ...
  if (STU_UNLIKELY(styleOverride)) {
    const Range<Int32> drawnRange =
        soOffsetRanges ? soOffsetRanges->drawnRangeInOriginalString : styleOverride->drawnRangeInOriginalString;
    const Range<Int32> overrideRange =
        soOffsetRanges ? soOffsetRanges->overrideRangeInOriginalString : styleOverride->overrideRangeInOriginalString;
    if (!drawnRange.overlaps(span.stringRange))
      return {};
    if (!drawnRange.contains(span.stringRange) || overrideRange.overlaps(span.stringRange)) {
      if (!overrideRange.contains(span.stringRange)) {
        return forEachStyledGlyphSpanSubspan(span, minX, maxX, *style, *styleOverride, flagsTestMask, body);
      }
      const TextFlags flags = (style->flags() & styleOverride->flagsMask) | styleOverride->flags;
      if (!(flagsTestMask & (flags | everyRunFlag)))
        return {};
  ...
  if (!(flagsTestMask & (style->flags() | everyRunFlag)))
    return {};
CallBody:
...
ShouldStop TextFrameLine::forEachStyledGlyphSpan(
    TextFlags flagsFilterMask,
    Optional<TextStyleOverride &> styleOverride,
    FunctionRef<ShouldStop(const StyledGlyphSpan &, const TextStyle &, Range<Float64>)> body) const
{
    ...
    if (flagsFilterMask & flags) {
      ...
      for (Int i = 0; i <= lastRunIndex; ++i) {
        ...
        if (i != endRunIndex) {
          ...
          span.stringRange = runStringRange.intersection(
              !this->isTruncatedAsRightToLeftLine
                  ? Range{this->rangeInOriginalString.start, para.excisedRangeInOriginalString().start}
                  : Range{para.excisedRangeInOriginalString().end, this->rangeInOriginalString.end});
        }
        const Float64 nextX = i == lastRunIndex ? leftPartWidth : x + span.glyphSpan.typographicWidth();
        const auto shouldStop =
            forEachStyledGlyphSpanSubspan(span, x, nextX, InOut{style}, styleOverride, none, flagsFilterMask, body);
        if (shouldStop)
          return shouldStop;
        x = nextX;
      }
  ...
  if (this->hasTruncationToken) {
    StyleOverrideOffsetRanges sor{.drawnRangeInOriginalString = Range<Int32>{uninitialized},
    ...
    if (STU_UNLIKELY(styleOverride)) {
      ...
      sor.drawnRangeInOriginalString =
          styleOverride->drawnRange.rangeInTruncatedString() - span.startIndexOfTruncationTokenInTruncatedString;
      sor.overrideRangeInOriginalString =
          styleOverride->overrideRange.rangeInTruncatedString() - span.startIndexOfTruncationTokenInTruncatedString;
      tokenFlags = stu_label::effectiveTextFlags(
          tokenFlags, tokenRange, *styleOverride, sor.drawnRangeInOriginalString, sor.overrideRangeInOriginalString);
    }
    if (tokenFlags & flagsFilterMask) {
      ...
      for (Int i = 0; i <= lastTokenRunIndex; ++i) {
        ...
        const Float64 nextX =
            i == lastTokenRunIndex ? tokenEndX : min(x + span.glyphSpan.typographicWidth(), tokenEndX);
        const auto shouldStop =
            forEachStyledGlyphSpanSubspan(span, x, nextX, InOut{tokenStyle}, styleOverride, sor, flagsFilterMask, body);
        if (shouldStop)
          return shouldStop;
        x = nextX;
      }
    ...
    if (STU_UNLIKELY(styleOverride)) {
      const TextFrameCompactIndex hyphenIndex{this->rangeInOriginalString.end - 1, IsIndexOfInsertedHyphen{true}};
  ...
  for (Int i = startRunIndex; i <= lastRunIndex; ++i) {
    ...
    const Float64 nextX = i == lastRunIndex ? width : min(x + span.glyphSpan.typographicWidth(), width);
    const auto shouldStop =
        forEachStyledGlyphSpanSubspan(span, x, nextX, InOut{style}, styleOverride, none, flagsFilterMask, body);
    if (shouldStop)
      return shouldStop;
...
static Int findGlyphStringEndIndex(Int glyphIndex,
                                   ArrayRef<const Int> stringIndices,
                                   bool isRTL,
...
[[nodiscard]] static ShouldStop enumerateStyledLigatureSubspans(
    StyledGlyphSpan &span,
    Float64 minX,
    Float64 maxX,
    Range<Int> drawnRange,
    Range<Int> overrideRange,
    const TextStyle *__nullable nonOverrideStyle,
    const TextStyle *__nullable overrideStyle,
    bool firstGraphemeClusterIsDrawn,
    bool firstGraphemeClusterIsOverridden,
    ...
    FunctionRef<ShouldStop(const StyledGlyphSpan &, const TextStyle &, Range<Float64>)> body)
{
  ...
  for (Int i = -1;;) {
    ...
    while (++i < innerOffsetCount) {
      ...
      if ((spanIsDrawn != nextIsDrawn) | (spanIsOverridden != nextIsOverridden))
        break;
    }
    if (spanIsDrawn) {
      ...
      if (style) {
        ...
        const Float64 spanMinX = !leftEndOfLigatureIsClipped ? minX : minX + innerOffsets[i0];
        const bool rightEndOfLigatureIsClipped = i != innerOffsetCount;
        span.rightEndOfLigatureIsClipped = rightEndOfLigatureIsClipped;
        const Float64 spanMaxX = !rightEndOfLigatureIsClipped ? maxX : minX + innerOffsets[i];
        span.stringRange = Range<Int32>{graphemeClusterStringRanges[i0 + 1].convexHull(graphemeClusterStringRanges[i])};
        const auto shouldStop = body(span, *style, Range{spanMinX, spanMaxX});
        if (shouldStop)
          return shouldStop;
      }
    }
    if (i == innerOffsetCount)
      break;
...
[[nodiscard]] static STU_NO_INLINE ShouldStop
forEachStyledGlyphSpanSubspan(StyledGlyphSpan &span,
                              Float64 minX,
                              const Float64 maxX,
                              ...
                              FunctionRef<ShouldStop(const StyledGlyphSpan &, const TextStyle &, Range<Float64>)> body)
{
  ...
  const bool shouldCallBodyOnOverridden =
      !styleOverride.overrideRange.isEmpty() &&
      (flagsTestMask & ((textFlags & styleOverride.flagsMask) | styleOverride.flags));
  if (!(shouldCallBodyOnNonOverridden | shouldCallBodyOnOverridden))
    return {};
  ...
  const TextStyle *const overrideStyle = !shouldCallBodyOnOverridden || !overrideRange.overlaps(runStringRange)
                                             ? nullptr
  ...
  if (isNonMonotonic) {
    sortedStringIndices = TempArray<Int>{uninitialized, Count{stringIndices.count()}, sortedStringIndices.allocator()};
  ...
  for (Int glyphIndex = subspanStartIndex, sortedStringIndicesCursor = glyphIndex;;) {
    Range<Int> glyphStringRange{stringIndices[glyphIndex],
                                findGlyphStringEndIndex(glyphIndex,
                                                        stringIndices,
                                                        isRTL,
                                                        runStringRange.end,
                                                        sortedStringIndices.begin(),
                                                        InOut{sortedStringIndicesCursor})};
    ...
    bool glyphIsOverridden = overrideRange.overlaps(glyphStringRange);
    {
      ...
      const bool mayNeedToSplitLigature =
          (glyphIsDrawn & !glyphIsFullyDrawn) | (glyphIsOverridden & !glyphIsFullyOverridden);
    ...
    if (ligatureInnerPositionCount == 0 && glyphIndex != subspanStartIndex &&
        ((subspanIsDrawn == glyphIsDrawn) &
         (subspanIsOverridden == glyphIsOverridden))) { // The current span continues.
      subspanStringRange = subspanStringRange.convexHull(glyphStringRange);
      ++glyphIndex;
      if (glyphIndex < glyphIndexRange.end)
        continue;
    }
    if (subspanStartIndex < glyphIndex) {
      span.glyphSpan = GlyphSpan{run, {subspanStartIndex, glyphIndex}, unchecked};
      const Float64 nextX =
          glyphIndex == glyphIndexRange.end ? maxX : min(minX + span.glyphSpan.typographicWidth(), maxX);
      if (subspanIsDrawn) {
    ...
    span.glyphSpan = GlyphSpan{run, {glyphIndex, glyphIndex + 1}, unchecked};
    const Float64 nextX =
        glyphIndex + 1 == glyphIndexRange.end ? maxX : min(minX + span.glyphSpan.typographicWidth(), maxX);
    const auto shouldStop = enumerateStyledLigatureSubspans(span,
                                                            minX,
                                                            nextX,
                                                            drawnRange,
                                                            overrideRange,
                                                            ...
                                                            glyphIsDrawn,
                                                            glyphIsOverridden,
                                                            graphemeClusterStringRanges,
                                                            ligatureInnerOffsets,
                                                            ligatureInnerPositionCount,
                                                            body);
    if (shouldStop)
      return shouldStop;
    if (++glyphIndex == glyphIndexRange.end)
