...
const Int32 TextStyle::stringIndexMask[2] = {TextStyle::maxSmallStringIndex, IntegerTraits<Int32>::max};

static_assert(alignof(TextStyle) == 4 && sizeof(TextStyle::LinkInfo) % 4 == 0 &&
              sizeof(TextStyle::BackgroundInfo) % 4 == 0 && sizeof(TextStyle::ShadowInfo) % 4 == 0 &&
              sizeof(TextStyle::UnderlineInfo) % 4 == 0 && sizeof(TextStyle::StrikethroughInfo) % 4 == 0 &&
              sizeof(TextStyle::StrokeInfo) % 4 == 0 && sizeof(TextStyle::AttachmentInfo) % 4 == 0 &&
...
{
  return sizeof(TextStyle) + ((flags & 1) ? sizeof(TextStyle::Big) - sizeof(TextStyle) : 0) +
...
void TextStyleOverride::applyTo(const TextStyle &style)
{
  ...
  const bool isBig = stringIndex > TextStyle::maxSmallStringIndex || fontIndex.value > TextStyle::maxSmallFontIndex ||
                     colorIndex.value > TextStyle::maxSmallColorIndex;

  using BitIndex = TextStyle::BitIndex;

  style_ = TextStyle::Big{isBig | (1 << TextStyle::BitIndex::isOverride) | (UInt64(effectiveFlags) << BitIndex::flags) |
  ...
  if (!(effectiveFlags & (TextFlags::hasBackground | TextFlags::hasShadow | TextFlags::hasUnderline |
                                            ...
                                            Optional<const TextFrameDrawingOptions &> options)
{
      ...
      if (const Optional<STUTextFrameRange> range = options->highlightTextFrameRange()) {
        ...
        if (textRange.type == STURangeInOriginalString) {
          ...
          highlightRangeInOriginalString =
              drawnRangeInOriginalString.intersection(clampToInt32IndexRange(textRange.range));
        }
  ...
  Range<Int32> drawnLineRange = {sign_cast(drawnRange.start.lineIndex), sign_cast(drawnRange.end.lineIndex)};
  ...
  return {drawnLineRange,
          drawnRangeInOriginalString,
          drawnRange,
          highlightRangeInOriginalString,
          highlightRange,
