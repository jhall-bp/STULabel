...
TempArray<TextLineSpan> TextFrame::lineSpans(STUTextFrameRange range,
                                             Optional<FunctionRef<bool(const TextStyle &)>> predicate) const
{
  ...
  for (auto &line : lines()[styleOverride.drawnLineRange]) {
    ...
    if (lineWidth > 0) {
      ...
      if (!rangeInTruncatedString.contains(r))
        continue;
      if (predicate && !predicate(firstNonTokenTextStyleForLineAtIndex(line.lineIndex)))
        continue;
...
Int adjustTextLineSpansByHorizontalInsetsAndReturnNewCount(ArrayRef<TextLineSpan> spans, HorizontalInsets insets)
{
  Int n = 0;
...
void extendTextLinesToCommonHorizontalBounds(ArrayRef<TextLineSpan> spans)
{
  ...
  if (minX == infinity<Float64> && maxX == -infinity<Float64>)
    return;
  for (auto &span : spans) {
    if (span.isLeftEndOfLine) {
      span.x.start = minX;
...
static TempArray<TaggedStringRange> findTaggedStringRanges(ArrayRef<const TextFrameParagraph> paragraphs,
                                                           ...
                                                           Optional<FunctionRef<bool(UInt, UInt)>> tagEquality)
{
  ...
  for (const TextFrameParagraph &para : paragraphs) {
    ...
    para.forEachStyledStringRange(styleOverride, [&](const TextStyle &style, StyledStringRange range) {
      if (!(tagTextFlagsMask & (style.flags() | everyRunFlag)))
        return;
      const Range<Int32> rangeInTruncatedString = range.stringRange + range.offsetInTruncatedString;
      if (!rangeInTruncatedString.overlaps(fullRangeInTruncatedString))
        return;
      ...
      buffer.append(TaggedStringRange{
          ...
          .rangeInOriginalString =
              !range.isTruncationTokenRange ? range.stringRange : para.excisedRangeInOriginalString(),
          .paragraphIndex = para.paragraphIndex,
          .tagIndex = tagIndex,
          .tag = tag,
          .taggedNonOverriddenStylePointer_ = !style.isOverrideStyle()
                                                  ? reinterpret_cast<UInt>(&style)
...
static Int32
indexOfTaggedRange(Int32 indexInTruncatedString, ArrayRef<const TaggedStringRange> ranges, Int32 previousRangeIndex)
{
...
static TempVector<TextLineSpan> findTaggedLineSpansForRanges(ArrayRef<const TextFrameParagraph> paragraphs,
  ...
  for (const TextFrameParagraph &para : paragraphs) {
    ...
    for (const TextFrameLine &line : lines[para.lineIndexRange().intersection(lineIndexRange)]) {
      ...
      if (STU_UNLIKELY(lineWidth <= 0)) {
        ...
        const Int32 taggedRangeIndex =
            indexOfTaggedRange(line.rangeInTruncatedString.start, ranges, previousTaggedRangeIndex);
        if (taggedRangeIndex < 0)
          continue;
        previousTaggedRangeIndex = taggedRangeIndex;
        const Range<Int32> r = line.rangeInTruncatedStringIncludingTrailingWhitespace();
        TaggedStringRange &range = ranges[taggedRangeIndex];
        if (!range.rangeInTruncatedString.contains(r))
          continue;
        range.hasSpan = true;
      ...
      line.forEachStyledGlyphSpan(
          styleOverride, [&](const StyledGlyphSpan &span, const TextStyle &style, Range<Float64> x) {
            if (!(tagStyleFlagMask & (style.flags() | everyRunFlag)))
              return;
            if (x.isEmpty())
              return;
            if (buffer.spans.count() == IntegerTraits<Int32>::max)
              return;
            const Int32 indexInTruncatedString =
                span.rangeInTruncatedString().start - (span.part == TextLinePart::insertedHyphen);
            const Int32 taggedRangeIndex = indexOfTaggedRange(indexInTruncatedString, ranges, previousTaggedRangeIndex);
            if (taggedRangeIndex < 0)
              return;
            previousTaggedRangeIndex = taggedRangeIndex;
            TaggedStringRange &range = ranges[taggedRangeIndex];
...
TaggedRangeLineSpans findAndSortTaggedRangeLineSpans(ArrayRef<const TextFrameLine> lines,
                                                     Optional<TextStyleOverride &> styleOverride,
                                                     TextFlags tagStyleFlagMask,
                                                     SeparateParagraphs separateParagraphs,
                                                     FunctionRef<UInt(const TextStyle &style)> tagger,
...
{
  if (lines.isEmpty())
    return TaggedRangeLineSpans{};
  const TextFrame &textFrame = lines[0].textFrame();
  const auto paragraphs = textFrame.paragraphs()[{lines[0].paragraphIndex, lines[$ - 1].paragraphIndex + 1}];
  const Range<Int32> lineIndexRange = {lines[0].lineIndex, lines[$ - 1].lineIndex + 1};
  const Range<Int32> rangeInTruncatedString = {lines[0].rangeInTruncatedString.start,
                                               lines[$ - 1].rangeInTruncatedString.end};
  TempArray<TaggedStringRange> ranges = findTaggedStringRanges(
      paragraphs, rangeInTruncatedString, styleOverride, tagStyleFlagMask, separateParagraphs, tagger, tagEquality);
  TempVector<TextLineSpan> spans =
      findTaggedLineSpansForRanges(paragraphs, lineIndexRange, styleOverride, Ref{ranges}, tagStyleFlagMask);
  ...
  if (!spans.isEmpty()) {
    ...
    std::sort(spans.begin(), spans.end(), [&](const TextLineSpan &span1, const TextLineSpan &span2) -> bool {
