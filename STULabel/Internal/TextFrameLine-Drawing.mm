...
static void drawRunGlyphsDirectly(GlyphSpan span, const TextStyle &style, DrawingContext &context)
{
  ...
  CTFontDrawGlyphs(font.ctFont(), gwp.glyphs().begin(), gwp.positions().begin(), sign_cast(gwp.count()), cgContext);
  if (stroke) {
    CGContextSetTextDrawingMode(cgContext, kCGTextFill);
  }
}

static void drawRunGlyphs(GlyphSpan glyphSpan, const TextStyle &style, CGFloat ctLineXOffset, DrawingContext &context)
{
...
static void drawGlyphs(const TextFrameLine &line, bool drawShadow, DrawingContext &context)
{
    ...
    line.forEachStyledGlyphSpan(
        ...
        [&](const StyledGlyphSpan &span, const TextStyle &style, Range<Float64> x) -> ShouldStop {
          if (style.flags() & TextFlags::hasAttachment) {
            ...
            drawAttachment(style.attachmentInfo()->attribute,
                           narrow_cast<CGFloat>(x.start),
                           style.baselineOffset(),
                           span.glyphSpan.count(),
                           context);
  ...
  line.forEachCTLineSegment(
      ...
      [&](TextLinePart part, CTLineXOffset ctLineXOffset, CTLine &ctLine, Optional<GlyphSpan> optGlyphSpan)
          -> ShouldStop {
        if (!optGlyphSpan) {
          ...
          CGContextSetTextMatrix(
              cgContext,
              CGAffineTransform{
                  .a = 1, .d = 1, .tx = context.lineOrigin().x + ctLineXOffset.value, .ty = context.lineOrigin().y});
...
static GlyphsShadowDrawingMode determineShadowDrawingMode(const TextFrameLine &line,
                                                          ...
                                                          const DrawingContext &context)
{
  ...
  line.forEachStyledStringRange(styleOverride, [&](const TextStyle &style, StyledStringRange) -> ShouldStop {
    ...
    const TextStyle::StrokeInfo *const stroke = style.strokeInfo();
    shouldDrawShadowSeparately |= stroke && (stroke->doNotFill == false || shadow->offsetX - shadow->blurRadius < 0);
    if (!glyphBoundsLLO) {
  ...
  return shouldDrawShadowSeparately ? shouldDrawGlyphsShadowSeparately : canDrawGlyphsTogetherWithShadow;
}
...
void TextFrameLine::drawLLO(DrawingContext &context) const
{
  ...
  const Optional<Underlines> underlines =
      flags & TextFlags::hasUnderline ? Underlines::find(*this, context) : Optional<Underlines>();
  ...
  if (glyphsDrawingMode == canDrawGlyphsTogetherWithShadow || !glyphsBoundingRectLLO ||
