...
void AttributeScanContext::scanAttribute(const void *keyPointer, const void *value, void *ctx)
{
  ...
  case 11: // NSUnderline
    if (equal(key, NSUnderlineStyleAttributeName)) {
      STU_CHECK_MSG(CFNumberGetValue((CFNumberRef)value, kCFNumberNSIntegerType, &context.underlineStyle),
  ...
  case 15: // STUOriginalFont
    ...
    if (equal(key, NSStrikethroughStyleAttributeName)) {
      STU_CHECK_MSG(CFNumberGetValue((CFNumberRef)value, kCFNumberNSIntegerType, &context.strikethroughStyle),
  ...
  case 16: // NSParagraphStyle
           // NSUnderlineColor
           // NSBaselineOffset
    if (equalAndNot(key, NSParagraphStyleAttributeName, NSUnderlineColorAttributeName, NSBaselineOffsetAttributeName)) {
...
FontIndex TextStyleBuffer::addFont(FontRef font)
{
  ...
  if (fontIndices_.count() == 0) {
    ...
    if (const auto [i, inserted] = fontIndices_.insert(
            hashPointer(ctFont), newIndex, [&](UInt16 index) { return fonts_[index].ctFont() == ctFont; });
        !inserted) {
      return FontIndex{i};
    }
  }
...
{
  return colorIndex == ColorIndex::black
             ? TextFlags{}
             : colors()[colorIndex.value - ColorIndex::fixedColorIndexRange.end].textFlags();
}

TextFlags TextStyleBuffer::encodeStringRangeStyle(Range<Int> range,
                                                  NSDictionary<NSAttributedStringKey, id> *__unsafe_unretained
                                                      __nullable attributes,
                                                  Optional<Out<ParagraphAttributes>> outParaAttributes)
{
  ...
  STU_DEBUG_ASSERT(lastStyleSize_ == 0 || reinterpret_cast<const Byte *>(lastStyle_) + lastStyleSize_ == data().end());
  void *next = data_.append(repeat(uninitialized, TextStyle::maxSize));
  lastStyle_ = reinterpret_cast<const TextStyle *>(reinterpret_cast<const Byte *>(next) - lastStyleSize_);

  UIFont *__unsafe_unretained font =
      STU_LIKELY(context.flags & Context::hasFont) ? context.font : (__bridge UIFont *)defaultCoreTextFont();
  const FontIndex fontIndex = addFont(font);
  const ColorIndex textColorIndex =
      !(context.flags & Context::hasForegroundColor) ? ColorIndex::black : addColor(context.foregroundColor);
  ...
  const bool isBig = range.end > TextStyle::maxSmallStringIndex || fontIndex.value > TextStyle::maxSmallFontIndex ||
  ...
  if (context.flags & ~(Context::hasFont | Context::hasForegroundColor)) {
    ...
    if (context.flags & Context::hasBackground) {
      if (STU_UNLIKELY(context.background->_color == nil &&
                       (context.background->_borderColor == nil || context.background->_borderWidth <= 0))) {
        context.background = nil;
      } else {
        ...
        auto *const info = new (next) TextStyle::BackgroundInfo{
            .stuAttribute = context.background, .colorIndex = colorIndex, .borderColorIndex = borderColorIndex};
        next = info + 1;
      }
    ...
    if (context.flags & Context::hasShadow) {
      ...
      if (color) {
        ...
        flags |= colorFlags(colorIndex);
        auto *const info =
            new (next) TextStyle::ShadowInfo{.offsetX = narrow_cast<Float32>(clampFloatInput(offset.width)),
    ...
    if ((context.flags & Context::hasUnderlineStyle) && (context.underlineStyle & 0xf)) {
      ...
      auto *const info = new (next) TextStyle::UnderlineInfo{context.underlineStyle, colorIndex, *cachedFontInfo};
      next = info + 1;
    }
    ...
    if (context.flags & Context::hasStrokeWidth) {
      const Float32 strokeWidth = narrow_cast<Float32>((1 / CGFloat(100)) * clampFloatInput(context.strokeWidth) *
    ...
    if (context.flags & (Context::hasTextAttachment | Context::hasNSTextAttachment)) {
      if (!(context.flags & Context::hasTextAttachment)) {
        STUImageTextAttachment *const imageAttachment =
            [[STUImageTextAttachment alloc] initWithNSTextAttachment:context.nsTextAttachment stringRepresentation:nil];
        if (imageAttachment) {
      ...
      if (context.flags & Context::hasTextAttachment) {
        if (!(context.flags & Context::hasRunDelegate) || range.count() > 1) // rdar://36622225
        {
          needToFixAttachmentAttributes_ = true;
        }
  ...
  if (size == lastStyleSize_ && flags == lastStyle_->flags() && fontIndex == lastStyle_->fontIndex() &&
      textColorIndex == lastStyle_->colorIndex()) {
    if (firstInfoOffset == size || memcmp(reinterpret_cast<Byte *>(style) + firstInfoOffset,
  ...
  style->bits = isBig | (static_cast<UInt64>(flags) << TextStyle::BitIndex::flags) |
...
STU_NO_INLINE
void TextStyleBuffer ::fixAttachmentAttributesIn(NSMutableAttributedString *__nonnull attributedString)
{
  ...
  for (;;) {
    ...
    if (auto *info = style->attachmentInfo()) {
      ...
      const auto attributes = [attributedString attributesAtIndex:stringRange.location effectiveRange:nil];
      if (![attributes objectForKey:STUAttachmentAttributeName]) {
        [attributedString
            addAttributes:@{STUAttachmentAttributeName : attachment, runDelegateKey : [attachment newCTRunDelegate]}
                    range:stringRange];
        decrementRefCount(attachment); // See the line above marked with ***.
      } else if (![attributes objectForKey:runDelegateKey]) {
        [attributedString addAttribute:runDelegateKey
