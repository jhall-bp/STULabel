...
static void addLine(PathIntersectionXBoundsState &s, CGPoint a, CGPoint b)
{
  ...
  for (;;) {
    ...
    if (stripeMinY == s.yis.upper.start)
      return;
...
static void addQuadraticCurve(PathIntersectionXBoundsState &s, CGPoint a, CGPoint b, CGPoint c, size_t depth)
{
  const CGFloat maxY = s.yis.upper.end;
  const CGFloat minY = s.yis.lower.start;
  if (a.y > maxY && b.y > maxY && c.y > maxY)
    return;
  if (a.y < minY && b.y < minY && c.y < minY)
    return;
...
static void addCubicCurve(PathIntersectionXBoundsState &s, CGPoint a, CGPoint b, CGPoint c, CGPoint d, size_t depth)
{
  const CGFloat maxY = s.yis.upper.end;
  const CGFloat minY = s.yis.lower.start;
  if (a.y > maxY && b.y > maxY && c.y > maxY && d.y > maxY)
    return;
  ...
  const CGFloat eeff = max(e.x * e.x, f.x * f.x) + max(e.y * e.y, f.y * f.y);
...
LowerAndUpperInterval findXBoundsOfPathIntersectionWithHorizontalLines(__nonnull CGPathRef path,
                                                                       Range<CGFloat> lowerLineY,
                                                                       Range<CGFloat> upperLineY,
                                                                       CGFloat maxError)
{
  ...
  PathIntersectionXBoundsState state = {.yis = {.lower = lowerLineY, .upper = upperLineY},
                                        .xis = {Range<CGFloat>::infinitelyEmpty(), Range<CGFloat>::infinitelyEmpty()},
