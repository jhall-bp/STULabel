...
STU_NO_INLINE
void drawUsingSTUTextFrame(NSAttributedString *__unsafe_unretained attributedString, CGSize size, CGPoint offset){
...
STU_NO_INLINE
void drawUsingNSStringDrawing(NSAttributedString *__unsafe_unretained attributedString, CGSize size, CGPoint offset){
    {const NSStringDrawingOptions options = NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading |
...
STU_NO_INLINE
void drawUsingTextKit(NSAttributedString *__unsafe_unretained attributedString, CGSize size, CGPoint offset)
{
  {
...
STU_NO_INLINE
void drawUsingCTLine(NSAttributedString *__unsafe_unretained attributedString, CGSize __unused size, CGPoint offset){
...
const CGPoint p = {.x = offset.x + (isRTL ? size.width - (CGFloat)width : 0), .y = offset.y + ascent};
...
void drawUsingCTTypesetter(NSAttributedString *__unsafe_unretained attributedString,
                           CGSize __unused size,
                           CGPoint offset){{const CFIndex stringLength = (CFIndex)attributedString.length;
if (stringLength == 0)
  return;
...
const CTTypesetterRef ts = CTTypesetterCreateWithAttributedString((__bridge CFAttributedStringRef)attributedString);
...
for (lineIndex = 0; stringIndex < stringLength && lineIndex < maxLineCount; ++lineIndex) {
  ...
  if (truncationStart < 0) {
    ...
    const CTLineRef untruncated = CTTypesetterCreateLine(ts, (CFRange){stringIndex, stringLength - stringIndex});
    ...
    NSAttributedString *const tokenString = [[NSAttributedString alloc] initWithString:@"…" attributes:attributes];
    const CTLineRef token = CTLineCreateWithAttributedString((CFAttributedStringRef)tokenString);
    line = CTLineCreateTruncatedLine(untruncated, size.width, kCTLineTruncationEnd, token);
    CFRelease(token);
...
for (lineIndex = 0; lineIndex < lineCount; ++lineIndex) {
  const CGPoint p = {.x = offset.x + origins[lineIndex].x, .y = offset.y + origins[lineIndex].y};
  CGContextSetTextMatrix(context, (CGAffineTransform){.a = 1, .d = 1, .tx = p.x, .ty = -p.y});
...
STU_NO_INLINE
void drawUsingCTFrame(NSAttributedString *__unsafe_unretained attributedString, CGSize size, CGPoint offset)
{
  {
    ...
    if (endStringIndex == stringLength) {
      ...
      CGContextConcatCTM(context, (CGAffineTransform){.a = 1, .d = -1, .tx = offset.x, .ty = size.height + offset.y});
      ...
      const CTLineRef untruncated = CTTypesetterCreateLine(
          ts, (CFRange){lastLineStringRange.location, stringLength - lastLineStringRange.location});
      NSDictionary<NSAttributedStringKey, id> *const attributes =
          [attributedString attributesAtIndex:(NSUInteger)endStringIndex effectiveRange:0];
      NSAttributedString *const tokenString = [[NSAttributedString alloc] initWithString:@"…" attributes:attributes];
      const CTLineRef token = CTLineCreateWithAttributedString((CFAttributedStringRef)tokenString);
      lastLine = CTLineCreateTruncatedLine(untruncated, size.width, kCTLineTruncationEnd, token);
      CFRelease(token);
      CFRelease(untruncated);
      ...
      {
        const bool isLTR = CTRunGetStringRange(CFArrayGetValueAtIndex(CTLineGetGlyphRuns(lastLine), 0)).location ==
