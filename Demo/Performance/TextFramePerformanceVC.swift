...
class TextFramePerformanceTestCase {
  ...
  init(
    ...
    let info1 = STUTextFrame(
      ...
      options: nil
    ).layoutInfo(frameOrigin: .zero)
    let info2 = STUTextFrame(
      STUShapedString(
        attributedString2,
        defaultBaseWritingDirection: .leftToRight),
      size: CGSize(width: width ?? 1000, height: height ?? 1000),
    ...
    self.size = CGSize(
      width: width
        ?? ceil(
          max(
            info1.layoutBounds.size.width,
            info2.layoutBounds.size.width)),
      height: height
        ?? ceil(
          max(
            info1.layoutBounds.size.height,
...
let cachedContext: CGContext = {
  ...
  let context = CGContext(
    data: nil, width: width * intScale, height: height * intScale,
...
func createContext(_ size: CGSize) -> CGContext {
  ...
  let context = CGContext(
    data: cachedContext.data!, width: Int(width), height: Int(height),
    bitsPerComponent: bitsPerComponent, bytesPerRow: bytesPerRow,
    space: space,
    bitmapInfo: bitmapInfo.rawValue)!
  context.concatenate(
    CGAffineTransform(
      a: benchmarkDisplayScale, b: 0,
  ...
  -> (Stats, UIImage)
{
  ...
  while true {
    ...
    autoreleasepool {
      ...
      autoreleasepool {
        draw(
          (i & 1) == 0 ? attributedString1 : attributedString2,
  ...
  let image = UIImage(
    cgImage: createContext(contextSize).makeImage()!, scale: benchmarkDisplayScale,
    orientation: .up)
...
class TextFramePerformanceVC: UIViewController, UIPopoverPresentationControllerDelegate {
  ...
  class NamedDisplayer: Equatable, Hashable {
    ...
    init(
      _ function: @escaping DrawingFunction,
    ...
    func createImage(_ testCase: TextFramePerformanceTestCase) -> UIImage {
      let size = CGSize(
        width: testCase.size.width + 2 * xInset,
  ...
  private class ResultView: TimingResultView<UIImageView> {
    ...
    func updateSampleViews() {
      let activeSampleViews = Set(
        sampleViewsByDisplayer.lazy.filter { $0.key.enabled.value }
          .map { $0.value })
  ...
  init() {
    ...
    let multiLineDisplayers: [NamedDisplayer] = [
      .init(
        drawUsingCTTypesetter, "CTTypesetter",
        enabledByDefault: true),
      .init(
        drawUsingCTFrame, "CTFrame",
        enabledByDefault: false),
      .init(
        drawUsingSTUTextFrame, "STUTextFrame",
        enabledByDefault: true),
      .init(
        measureAndDrawUsingNSStringDrawing, "NSStringDrawing", "measuring & drawing",
        enabledByDefault: true),
      .init(
        drawUsingNSStringDrawing, "NSStringDrawing", "drawing only",
        enabledByDefault: true),
      .init(
        drawUsingTextKit, "TextKit",
        enabledByDefault: true),
    ]
    let displayers: [NamedDisplayer] =
      [
    ...
    for test in tests {
      views.append(
        ResultView(
          test,
          test.lineCount == 1 && !test.needsTruncation
            ? displayers
            : multiLineDisplayers))
    }
  ...
  private func measureDrawingTimes(_ view: ResultView) {
    ...
    guard !displayers.isEmpty
      ...
      let ac = UIAlertController(
        title: "Error", message: "No compatible drawing function is enabled. Please select one.",
    ...
    func measure(_ index: Int) {
      ...
      DispatchQueue.main.asyncAfter(deadline: .now() + max(pause, index == 0 ? 0.5 : 0.1)) {
        ...
        print(
          function.name + (function.features.isEmpty ? "" : " (" + function.features + ")"),
