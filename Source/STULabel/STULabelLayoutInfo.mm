...
const LabelTextFrameInfo LabelTextFrameInfo::empty = {.isValid = true, .flags = STUTextFrameHasMaxTypographicWidth};
                                      ...
                                      const DisplayScale &displayScale)
{
  ...
  if (frame.lineCount != 0) {
    ...
    maxY = lastBaseline + (frame.layoutMode == STUTextLayoutModeDefault
  ...
  CGSize minFrameSize = {min(frame.size.width, width),
                         min(frame.size.height, narrow_cast<CGFloat>(maxYWithoutSpacingBelowLastLine) - y)};
...
STULabelLayoutInfo
stuLabelLayoutInfo(const LabelTextFrameInfo &info, CGPoint textFrameOrigin, const stu_label::DisplayScale &displayScale)
{
  return {
...
bool LabelTextFrameInfo::isValidForSizeImpl(CGSize size, const DisplayScale &displayScale) const
{
  ...
  return isValid && size.width >= minFrameSize.width && size.height >= minFrameSize.height &&
...
STU_NO_INLINE
CGSize LabelTextFrameInfo::sizeThatFits(const UIEdgeInsets &insets, const DisplayScale &displayScale) const
{
  return ceilToScale(layoutBounds.inset(-roundLabelEdgeInsetsToScale(insets, displayScale)), displayScale).size();
}

STU_NO_INLINE
CGPoint textFrameOriginInLayer(const LabelTextFrameInfo &info, const LabelParameters &p)
{
