...
STU_EXPORT STU_NO_INLINE void stu_mutex_destroy(stu_mutex *__nonnull mutex) {
  if (&os_unfair_lock_lock)
    return;
  pthread_mutex_destroy(&mutex->pthread_mutex);
}

STU_EXPORT STU_NO_INLINE bool stu_mutex_trylock(stu_mutex *__nonnull mutex) {
...
STU_EXPORT STU_NO_INLINE void stu_mutex_lock(stu_mutex *__nonnull mutex) {
  if (&os_unfair_lock_lock) {
...
STU_EXPORT STU_NO_INLINE void stu_mutex_unlock(stu_mutex *__nonnull mutex) {
