...
static Int
trailingRunIndex(const NSArrayRef<CTRun *> &runs, Int stringEndIndex, STUWritingDirection baseWritingDirection)
{
  ...
  for (Int i = start; i != end; i += d) {
    ...
    if (stringRange.end < maxStringRangeEnd)
      continue;
    ...
    if (stringRange.end >= stringEndIndex)
      return i;
...
auto TextFrameLayouter ::breakLineAt(TextFrameLine &line,
                                     Int stringIndex,
                                     Hyphen hyphen,
    ...
    -> BreakLineAtStatus
{
  ...
  if (stringLength > 0) {
    ctLine = CTTypesetterCreateLineWithOffset(
        typesetter_, Range{line.rangeInOriginalString.start, stringIndex}, lineHeadIndent_);
    ...
    if (STU_UNLIKELY(width <= 0)) {
      ...
      const bool mayNeedToCorrectLastGlyphAdvance =
          trailingWhitespaceStringLength == 0 && stringIndex < attributedString_.string.count();
      const NSArrayRef<CTRun *> runs =
          mayNeedToCorrectLastGlyphAdvance || hyphen != 0u ? glyphRuns(ctLine) : NSArrayRef<CTRun *>{};
      if (hyphen != 0u) {
        if (const Int trailingRunIndex =
                stu_label::trailingRunIndex(runs, stringIndex, line.paragraphBaseWritingDirection);
            trailingRunIndex >= 0) {
          ...
          line.init_step2(TextFrameLine::InitStep2Params{
              ...
              .token = {
                  ...
                  .hyphen = {.runIndex = hyphenLine.runIndex,
      ...
      if (mayNeedToCorrectLastGlyphAdvance && !runs.isEmpty()) {
        ...
        const auto glyph = GlyphForKerningPurposes::find(runs[$ - 1], attributedString_, rightmostGlyph);
...
void TextFrameLayouter::justifyLine(STUTextFrameLine &line) const
{
  ...
  const CTLine *justifiedCTLine =
      CTLineCreateJustifiedLine(line._ctLine, 1, lineMaxWidth_ - hyphenWidthPlusAdvanceCorrection);
  if (!justifiedCTLine)
    return;
  ...
  if (line.hasInsertedHyphen) {
    ...
    trailingRunIndex =
        stu_label::trailingRunIndex(runs, line.rangeInOriginalString.end, line.paragraphBaseWritingDirection);
...
bool TextFrameLayouter::hyphenateLineInRange(TextFrameLine &line, Range<Int> stringRange)
{
  if (lastHyphenationLocationInRangeFinder_) {
    for (Int i = stringRange.end; i > stringRange.start + 1;) {
      const STUHyphenationLocation hl = lastHyphenationLocationInRangeFinder_(attributedString_.attributedString,
      ...
      if (hl.index <= sign_cast(stringRange.start) || hl.index >= sign_cast(i))
        break;
  ...
  const Int outerEnd = stringRange.end >= stringLength
                           ? stringRange.end
                           : attributedString_.string.indexOfFirstUTF16CharWhere(
                                 {stringRange.end, min(stringRange.end + 16, stringLength)}, isUnicodeWhitespace);
  ...
  [attributedString_.attributedString
      enumerateAttribute:STUHyphenationLocaleIdentifierAttributeName
              ...
              usingBlock:^(__unsafe_unretained id value, NSRange nsRange, BOOL *shouldStop) {
                ...
                if (range.start >= stringRange.end)
                  return;
                CFString *const localeId = (__bridge CFStringRef)value;
                if (!localeId)
                  return;
                if (localeId != cachedLocaleId_ && CFStringGetLength(localeId) == 0)
                  return;
...
static bool isCodePointThatLikelyPrecedesGoodLineBreakLocation(Char32 ch)
{
  ...
  case 0x2014:
  case 0x2E3A:
  case 0x2E3B: // Break_Both (EM dashes)
  ...
  case 0x2008:
  case 0x2009:
  case 0x200A: // PUNCTUATION SPACE..HAIR SPACE
  case 0x2010: // HYPHEN
  case 0x2012:
  case 0x2013: // FIGURE DASH..EN DASH
...
static bool isLikelyGoodLineBreakLocation(const NSAttributedStringRef &attributedString, Int index)
{
  const Char16 ch = attributedString.string[index - 1];
  if (isCodePointThatLikelyPrecedesGoodLineBreakLocation(ch))
    return true;
  const Char16 ch1 = attributedString.string[index];
  switch (ch1) {
  case 0x2014:
  case 0x2E3A:
  case 0x2E3B: // The EM dashes.
  case 0xFFFC: // Object replacement character
    return true;
  default:
    return false;
...
void TextFrameLayouter::breakLine(TextFrameLine &line, Int paraStringEndIndex)
{
  ...
  Int end =
      min(paraStringEndIndex, start + CTTypesetterSuggestLineBreakWithOffset(typesetter_, start, maxWidth, headIndent));
  ...
  for (;;) {
    ...
    const auto status = breakLineAt(line, end1, Hyphen{hyphen}, TrailingWhitespaceStringLength{end - end1});
    if (status.success)
      break;
    STU_DEBUG_ASSERT(hyphen != 0);
    const Int end2 = start + CTTypesetterSuggestLineBreakWithOffset(
                                 typesetter_, start, status.ctLineWidthWithoutHyphen - 0.01, headIndent);
    if (start < end2 &&
        end2 < end
  ...
  const Int maxEnd = clamp(end,
                           start + CTTypesetterSuggestClusterBreakWithOffset(typesetter_, start, maxWidth, headIndent),
                           paraStringEndIndex);
  ...
  hyphenateLineInRange(line, Range{isGoodBreak ? end : start, string.endIndexOfGraphemeClusterAt(maxEnd)});
}
