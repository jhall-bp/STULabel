    ...
    -> Pair<STUFirstLineOffsetType, Float64>
{
  ...
  case STUBaselineAdjustmentAlignFirstLineCenter:
    return {STUOffsetOfFirstLineCenterFromTop,
            firstLine.originY - (firstLine._heightAboveBaseline - firstLine._heightBelowBaseline) / 2};
                                       ...
                                       const TextFrameOptions &options)
{
  ...
  state.scaleInfo = ScaleInfo{.originalDisplayScale = displayScale.storage().displayScaleOrZero(),
  ...
  const Int32 maxLineCount = options.maximumNumberOfLines > 0 && options.maximumNumberOfLines <= maxValue<Int32>
                                 ? narrow_cast<Int32>(options.maximumNumberOfLines)
                                 : maxValue<Int32>;
  ...
  layout(Size{frameSize.width, shouldEstimateScaleFactor ? unlimitedHeight : frameSize.height},
         state.scaleInfo,
         shouldEstimateScaleFactor ? maxValue<Int32> : maxLineCount,
         options);
  if (!shouldEstimateScaleFactor || isCancelled())
    return;
  inverselyScaledFrameSize_.height = frameSize.height;

  if (lines_.count() <= maxLineCount && !mayExceedMaxWidth_ && lastLineFitsFrameHeight())
    return;
  ...
  const CGFloat accuracy =
      hasStepSize ? stepSize : narrow_cast<CGFloat>(1 / clamp(32, 2 * max(frameSize.width, frameSize.height), 2048));
  ...
  const auto estimatedScale = minTextScaleFactor + stepSize >= 1
                                  ...
                                  : estimateScaleFactorNeededToFit(frameSize.height,
                                                                   maxLineCount,
  ...
  const auto [type, offset] = firstLineOffsetForBaselineAdjustment(lines_[0], state.scaleInfo.baselineAdjustment);
  ...
  const auto updateScaleInfo = [&](CGFloat scale) {
    ...
    if (state.scaleInfo.originalDisplayScale != 0) {
      ...
      state.maxInverselyScaledHeight =
          state.inverselyScaledFrameSize.height - state.scaleInfo.displayScale->inverseValue_f64();
    } else {
  ...
  const auto updateScaleInfoAndLayout = [&](CGFloat scale) {
    ...
    layout(Size{state.inverselyScaledFrameSize.width, height}, state.scaleInfo, lineCount, options);
    inverselyScaledFrameSize_.height = state.inverselyScaledFrameSize.height;
  };
  const auto fits = [&] {
    if (lines_.count() > maxLineCount)
      return false;
  ...
  const auto updateLowerBound = [&]() -> bool {
    ...
    if (lowerBoundPlusAccuracy >= state.upperBound || isCancelled())
      return false;
  ...
  if (!estimatedScale.isAccurate && fits()) {
    ...
    if (!updateLowerBound())
      return;
    nextScale = min(state.lowerBound + max(1 / 64.f, 2 * accuracy), (state.lowerBound + state.upperBound) / 2);
  } else {
    if (!updateUpperBound())
      return;
    if (hasStepSize) {
    ...
    nextScale = max(state.upperBound - max(1 / 64.f, 2 * accuracy), (state.lowerBound + state.upperBound) / 2);
  }
  for (;;) {
    if (hasStepSize) {
...
static Float64 computeWidth(CTTypesetter *typesetter, Range<Int32> stringRange, Float64 headIndent)
{
...
STU_NO_INLINE
Float64 TextFrameLayouter::estimateTailTruncationTokenWidth(
    const TextFrameLine &line, NSAttributedString *__unsafe_unretained originalTruncationToken) const
...
struct ScalingPara
{
  ...
  /// for hyphenation).
  void bisectInverseScaleInterval(bool lineCountIsLowerBound,
                                  ...
                                  const NSStringRef &string)
  {
    ...
    if (maxLineCount == 2 && commonHeadIndent == 0) {
      if (singleLineInverseScale == 0) {
        ...
        const Float64 w = computeWidth(typesetter, stringRange, max(0.f, initialExtraHeadIndent));
        singleLineInverseScale = (initialHeadIndent + initialTailIndent + w) / maxWidthMinusCommonIndent;
      }
    ...
    for (Int32 n = 1, index = stringRange.start, endIndex;; ++n, index = endIndex) {
      ...
      endIndex =
          index + narrow_cast<Int32>(CTTypesetterSuggestLineBreakWithOffset(typesetter, index, maxWidth, headIndent));
...
auto TextFrameLayouter::estimateScaleFactorNeededToFit(Float64 frameHeight,
                                                       Int32 maxLineCount,
                                                       NSAttributedString *__unsafe_unretained truncationToken,
                                                       Float64 minScale,
                                                       Float64 accuracy) const -> ScaleFactorEstimate
{
  ...
  if (STU_UNLIKELY(mayExceedMaxWidth_)) {
    ...
    for (auto &para : paras_) {
      if (STU_UNLIKELY(lineIndex == para.lineIndexRange().end))
        continue;
      ...
      if (STU_UNLIKELY(p.isIndented)) {
        ...
        initialExtraIndent = max(0.f, p.initialExtraLeftIndent) + max(0.f, p.initialExtraRightIndent);
        nonInitialExtraIndent = max(0.f, -p.initialExtraLeftIndent) + max(0.f, -p.initialExtraRightIndent);
      }
      if (maxWidth <= 0)
        return {minScale, true};
      ;
      do {
        const Float64 width = (lineIndex < para.initialLinesEndIndex ? initialExtraIndent : nonInitialExtraIndent) +
  ...
  const Float64 firstLineOffset =
      scaleInfo_.scale == 1 ? firstLineOffsetForBaselineAdjustment(lines[0], scaleInfo_.baselineAdjustment).second
  ...
  if (lineCount > maxLineCount) {
    ...
    if (newlineCount >= maxLineCount) {
      ...
      for (Int i0 = 0, i = 0; i < lines.count(); i0 = i) {
        do {
          if (lines[i++].isFollowedByTerminatorInOriginalString)
            break;
        } while (i < lines.count());
        if (i == i0 + 1)
          continue;
        ...
        if (STU_UNLIKELY(p.isIndented)) {
          ...
          headIndent = p.baseWritingDirection == STUWritingDirectionLeftToRight ? commonLeftIndent + extraIndentLeft
                                                                                : commonRightIndent + extraIndentRight;
        }
        if (maxWidth <= extraIndent || isCancelled())
          return {minScale, true};
        const Range<Int32> range{firstLine.rangeInOriginalString.start, lastLine.rangeInOriginalString.end};
        const Float64 width =
            extraIndent + (i == lines.count() ? lastLineExtraWidth : 0) + computeWidth(typesetter_, range, headIndent);
        if (width > 0) {
          scale = min(scale, maxWidth / width);
          if (scale <= minScale) {
            return {minScale, true};
        ...
        const Float32 lastLineHeightBelowBaseline =
            !lastLine.isLastLine ? lastLine._heightBelowBaseline : heightBelowBaselineWithoutExcessSpacing(lastLine);
        ...
        const Float64 d =
            (lastLine.originY - firstLine.originY) + (firstLine._heightAboveBaseline + lastLineHeightBelowBaseline -
  ...
  TempVector<ScalingPara> paras{freeCapacityInCurrentThreadLocalAllocatorBuffer, paras_.allocator()};
    ...
    -> ScaleFactorAndNeedsRealignment
{
  ...
  for (const TextFrameParagraph &para : paras_) {
    ...
    if (STU_UNLIKELY(i == para.lineIndexRange().end))
      continue;
