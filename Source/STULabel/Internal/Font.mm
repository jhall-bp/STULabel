...
alignas(FontInfoCache) Byte fontInfoCacheStorage[sizeof(FontInfoCache)];

CachedFontInfo::CachedFontInfo(FontRef font) : metrics{uninitialized}
{
...
alignas(GlyphBoundsCache) Byte glyphBoundsCacheStorage[sizeof(GlyphBoundsCache)];
...
HashCode<UInt> FontFaceGlyphBoundsCache::FontFace::hash()
{
  return narrow_cast<HashCode<UInt>>(
      stu_label::hash(bit_cast<UInt>(cgFont.get()) ^ hashableBits(fontMatrix.a) ^ hashableBits(fontMatrix.b),
                      hashableBits(appleColorEmojiSize) ^ hashableBits(fontMatrix.c) ^ hashableBits(fontMatrix.d)));
}

STU_NO_INLINE
void FontFaceGlyphBoundsCache::exchange(InOut<UniquePtr> inOutArg, FontRef font, FontFace &&fontFace)
{
  ...
  const auto isEqualFontFace = [&](const Malloced<Pool> &entry) { return fontFace == entry->fontFace; };
  ...
  const auto result = glyphBoundsCache.poolsByFontFace.insert(
      hashCode, isEqualFontFace, [&] { return mallocNew<Pool>(std::move(fontFace), font.ctFont()); });
...
STU_NO_INLINE
void FontFaceGlyphBoundsCache ::returnToGlobalPool(ArrayRef<FontFaceGlyphBoundsCache * __nullable const> caches)
{
  stu_mutex_lock(&glyphBoundsCacheMutex);
...
static CGFloat appleColorEmojiTrackValue(bool isAppleColorEmojiUI, CGFloat fontSize)
{
  ...
  static constexpr Entry uiTable[] = {{0, 124},
                                      {9, 124},
                                      {13, 116},
                                      {16, 114},
                                      {17, 112},
  ...
  for (Int i = 1;;) {
    ...
    return trunc(table[i - 1].value + ((fontSize - table[i - 1].fontSize) *
...
static CGPoint scaledAppleColorEmojiOffset(bool isAppleColorEmojiUI, CGFloat fontSize, CGFloat effectivePointsPerEM)
{
  ...
  CGFloat x = narrow_cast<CGFloat>(effectivePointsPerEM * track * 0.4);
...
FontFaceGlyphBoundsCache::InitData::InitData(Pool &pool)
    : pool{pool}, font{pool.ctFont.get()}, unitsPerEM{static_cast<CGFloat>(CTFontGetUnitsPerEm(font.ctFont()))},
      offset{}, isAppleColorEmoji{pool.fontFace.isAppleColorEmoji}, useIntBounds{pool.fontFace.fontMatrixIsIdentity}
{
  if (!isAppleColorEmoji) {
    ...
    offset = scaledAppleColorEmojiOffset(pool.fontFace.isAppleColorEmojiUI, fontSize, pointsPerUnit);
  }
}

FontFaceGlyphBoundsCache::FontFaceGlyphBoundsCache(InitData data)
    : pool_{data.pool}, font_{pool_.ctFont.get()}, unitsPerEM_{data.unitsPerEM}, pointsPerUnit_{data.pointsPerUnit},
      inversePointsPerUnit_{1 / pointsPerUnit_}, scaledIntBoundsOffset_{data.offset},
      isAppleColorEmoji_{data.isAppleColorEmoji}, usesIntBounds_{data.useIntBounds}
{
  if (usesIntBounds_) {
                                                     ...
                                                     const CGPoint *const positions)
{
  ...
  enum class BoundsAreInt : bool
  {
  }; // A "strong typedef" used in lieue of a named argument.

  const auto extendRect = [&](/* Rect */ auto glyphBounds, BoundsAreInt boundsAreInt, Int positionIndex)
                              STU_INLINE_LAMBDA {
  ...
  if (fontSize > 0) {
    ...
    const auto scanGlyphs = [&](auto &boundsByGlyph, const auto &placeholder) STU_INLINE_LAMBDA {
      for (Int i = 0; i < glyphs.count(); ++i) {
        ...
        const auto result = boundsByGlyph.insert(glyph, isEqualTo(glyph), [&] { return placeholder; });
  ...
  if (STU_UNLIKELY(!remaining.isEmpty())) {
    ...
    CTFontGetBoundingRectsForGlyphs(
        font_.ctFont(), kCTFontOrientationHorizontal, newGlyphs.begin(), newBounds.begin(), newGlyphCount);
    for (Int k = 0; k < newGlyphs.count(); ++k) {
      const CGGlyph glyph = newGlyphs[k];
      Rect<CGFloat> bounds =
          Rect{newBounds[k].origin - scaledIntBoundsOffset_, newBounds[k].size} * inversePointsPerUnit_;
      if (STU_LIKELY(usesIntBounds_)) {
        ...
        if (STU_LIKELY(
                ...
                && isBoundsRectEqualToRectWithHighAccuracy(pointsPerUnit_ * r + scaledIntBoundsOffset_, newBounds[k])
#if STU_DEBUG
    ...
    if (newGlyphCount < remaining.count()) {
      const auto extendRectForRemainingGlyphs = [&](auto &boundsByGlyph) STU_INLINE_LAMBDA {
        for (auto i : remaining) {
          ...
          if (i >= 0)
            continue; // Indices of duplicate glyph occurences are multiplied by -1.
          i = -i;
          const CGGlyph glyph = glyphs[i];
...
void LocalGlyphBoundsCache::glyphBoundsCacheFor_slowPath(FontRef font)
{
  ...
  for (;;) {
    if (caches_[i]) {
      if (caches_[i]->fontFace() == fontFace)
        break;
      if (++i != entryCount)
        continue;
