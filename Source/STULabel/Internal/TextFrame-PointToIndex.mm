...
auto TextFrame::rangeOfGraphemeClusterClosestTo(Point<Float64> point,
                                                ...
                                                CGFloat displayScaleValue) const -> GraphemeClusterRange
{
  ...
  Empty:
    ...
    return {.range = {index, index},
            .bounds = CGRectZero,
            .writingDirection =
                paragraphs().isEmpty() ? STUWritingDirectionLeftToRight : paragraphs()[0].baseWritingDirection,
  ...
  Range<Int> lineIndexRange =
      verticalSearchTable().indexRange(narrow_cast<Range<Float32>>(point.y - origin.y + Range{-e, e}));
  ...
  const auto updateClosestLineIndex = [&](const TextFrameLine &line) {
    ...
    if (squaredDistance < closestSquaredDistance ||
        (squaredDistance == closestSquaredDistance && yDistanceFromLineCenter < closestYDistanceFromLineCenter)) {
...
auto TextFrameLine::rangeOfGraphemeClusterAtXOffset(Float64 xOffset) const -> TextFrame::GraphemeClusterRange
{
  ...
  forEachStyledGlyphSpan(
      none, [&](const StyledGlyphSpan &span, const TextStyle &, Range<Float64> spanXOffset) -> ShouldStop {
        // We only need to look at a single span.
        if (!spanXOffset.contains(xOffset) && (xOffset < width || spanXOffset.end < width))
          return {};
        ...
        Float64 glyphXOffset = spanXOffset.start;
        {
          ...
          for (Float64 nextGlyphXOffset; glyphIndex < lastGlyphIndex; ++glyphIndex, glyphXOffset = nextGlyphXOffset) {
            nextGlyphXOffset = glyphXOffset + glyphSpan[{glyphIndex, Count{1}}].typographicWidth();
            if (xOffset < nextGlyphXOffset)
              break;
          }
        }
        ...
        const Int graphemeClusterCount =
        ...
        if (1 < graphemeClusterCount && graphemeClusterCount - 1 <= maxInnerOffsetCount) {
          ...
          if (span.glyphSpan.copyInnerCaretOffsetsForLigatureGlyphAtIndex(
            ...
            for (; i < graphemeClusterCount - 1; ++i) {
              if (innerOffset < ligatureInnerOffsets[i])
                break;
            }
        ...
        if (span.part == TextLinePart::originalString) {
          if (stringRange.start < para.excisedRangeInOriginalString().start) {
            stringRange.intersect(Range{rangeInOriginalString.start, para.excisedRangeInOriginalString().start});
            offsetInTruncatedString = this->rangeInTruncatedString.start - this->rangeInOriginalString.start;
          } else {
            stringRange.intersect(Range{para.excisedRangeInOriginalString().end, rangeInOriginalString.end});
            offsetInTruncatedString = this->rangeInTruncatedString.end - this->rangeInOriginalString.end;
          }
  ...
  if (xOffsetBounds.isEmpty()) {
    ...
    forEachStyledGlyphSpan(styleOverride, [&](const StyledGlyphSpan &span, const TextStyle &, Range<Float64> xOffset) {
      if (xOffsetBounds.isEmpty()) {
