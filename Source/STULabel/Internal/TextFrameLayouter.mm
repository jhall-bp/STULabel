                                                ...
                                                STUDefaultTextAlignment defaultTextAlignment)
{
  ...
  case NSTextAlignmentJustified:
    ...
    case STUDefaultTextAlignmentStart:
      result =
          writingDirection == STUWritingDirectionLeftToRight ? STUParagraphAlignmentLeft : STUParagraphAlignmentRight;
      break;
...
TextFrameLayouter::TextFrameLayouter(const ShapedString &shapedString,
    ...
    : TextFrameLayouter{InitData::create(shapedString, stringRange, defaultTextAlignment, cancellationFlag)}
{}

auto TextFrameLayouter::InitData::create(const ShapedString &shapedString,
                                         Range<Int32> stringRange,
                                         const STUDefaultTextAlignment defaultTextAlignment,
                                         Optional<const STUCancellationFlag &> cancellationFlag) -> InitData
{
  const ShapedString::ArraysRef sas = shapedString.arrays();
  const Int32 stringLength = shapedString.stringLength;

  STU_DEBUG_ASSERT(0 <= stringRange.start && stringRange.start <= stringRange.end && stringRange.end <= stringLength);
  ...
  const ArrayRef<const ShapedString::Paragraph> stringParas{stringParasBegin, stringParasEnd, unchecked};
  TempArray<TextFrameParagraph> paras{uninitialized, Count{stringParas.count()}};
  {
    ...
    for (TextFrameParagraph &para : paras) {
      ...
      new (&para) TextFrameParagraph{
          {.alignment = paragraphAlignment(p.alignment, p.baseWritingDirection, defaultTextAlignment),
           .rangeInOriginalString = p.stringRange,
           .excisedRangeInOriginalString = {p.stringRange.end, p.stringRange.end},
           .paragraphIndex = i,
           .paragraphTerminatorInOriginalStringLength = narrow_cast<UInt8>(p.terminatorStringLength),
  ...
  if (!paras.isEmpty()) {
    ...
    paras[$ - 1].paragraphTerminatorInOriginalStringLength = narrow_cast<UInt8>(
        max(0, stringRange.end - (stringParas[$ - 1].stringRange.end - stringParas[$ - 1].terminatorStringLength)));
  ...
  if (isFullString) {
    ...
    styles.firstStyle = reinterpret_cast<const TextStyle *>(sas.textStyles.dataBegin() + firstPara.textStylesOffset);
    ...
    if (stringParas.end() < sas.paragraphs.end()) {
      styles.terminatorStyle =
          reinterpret_cast<const TextStyle *>(sas.textStyles.dataBegin() + stringParas.end()->textStylesOffset);
    } else {
    ...
    if (stringRange.end < styles.terminatorStyle->stringIndex()) {
      do
        styles.terminatorStyle = &styles.terminatorStyle->previous();
...
TextFrameLayouter::TextFrameLayouter(InitData init)
    : tempStringBuffer_{std::move(init.tempStringBuffer)}, cancellationFlag_{init.cancellationFlag},
      typesetter_{init.typesetter}, attributedString_{init.attributedString}, originalStringStyles_{init.stringStyles},
      originalStringFontMetrics_{init.stringFontMetrics}, truncationScopes_{init.truncationScopes},
      stringParasPtr_{init.stringParas.begin()}, stringRange_{init.stringRange}, paras_{std::move(init.paras)},
      lines_{paras_.allocator()}, stringRangeIsFullString_{init.stringRangeIsFullString},
      clippedStringRangeEnd_{stringRange_.end}, clippedParagraphCount_{paras_.count()},
      clippedOriginalStringTerminatorStyle_{init.stringStyles.terminatorStyle},
      tokenStyleBuffer_{
          Ref{localFontInfoCache_}, paras_.allocator(), pair(init.stringColorInfos, init.stringColorHashBuckets)},
...
{
  if (!ownsCTLinesAndParagraphTruncationTokens_)
    return;
#if STU_DEBUG
...
void TextFrameLayouter::saveLayoutTo(SavedLayout &layout)
{
  ...
  const UInt size = sizeof(Data) + paras_.arraySizeInBytes() + lines_.arraySizeInBytes() +
  ...
  data->tokenStyleData = ArrayRef{reinterpret_cast<Byte *>(data->lines.end()), tokenStyleBuffer_.data().count()};
...
static TextFrameLine::HeightInfo
lineHeight(STUTextLayoutMode mode, const LineHeightParams &params, CGFloat ascent, CGFloat descent, CGFloat leading)
{
  ...
  case STUTextLayoutModeDefault: {
    ...
    const CGFloat hm = ad * params.lineHeightMultiple + max(g * params.lineHeightMultiple, params.minLineSpacing);
                                                       ...
                                                       const MinFontMetrics &minFontMetrics)
{
  ...
  if constexpr (mode == STUTextLayoutModeDefault) {
    ...
    return {.minHeight = narrow_cast<Float32>(h),
            ...
            .minHeightBelowBaselineWithoutSpacing = narrow_cast<Float32>(minHeightBelowBaselineWithoutSpacing),
...
template MinLineHeightInfo TextFrameLayouter ::minLineHeightInfo<STUTextLayoutModeDefault>(const LineHeightParams &,
                                                                                           const MinFontMetrics &);
template MinLineHeightInfo TextFrameLayouter ::minLineHeightInfo<STUTextLayoutModeTextKit>(const LineHeightParams &,
                                                                                           const MinFontMetrics &);

STU_INLINE
Float32 extraSpacingBeforeFirstAndAfterLastLineInParagraphDueToMinBaselineDistance(const STUTextFrameLine &line,
...
static Float64 calculateBaselineOfLineFromPreviousLine(const STUTextFrameLine *__nonnull const line,
                                                       ...
                                                       const TextFrameLayouter::ScaleInfo &scaleInfo)
{
  ...
  if (line->isFirstLineInParagraph) {
    ...
    if (line->lineIndex == 0) {
      ...
      if (minBaselineDistance == 0) {
        ...
        offset +=
            extraSpacingBeforeFirstAndAfterLastLineInParagraphDueToMinBaselineDistance(line1, minBaselineDistance);
    ...
    case STUOffsetOfFirstBaselineFromDefault: {
      ...
      if (para->minBaselineDistance > 0) {
        offset += extraSpacingBeforeFirstAndAfterLastLineInParagraphDueToMinBaselineDistance(*line,
  ...
  return line[-1].originY + max(line[-1]._heightBelowBaseline + line->_heightAboveBaseline, para->minBaselineDistance);
}

/// @pre scaleInfo == none if spara isn't the first paragraph.
STU_INLINE
Float32 minDistanceFromParagraphTopToSpacingBelowFirstBaseline(STUTextLayoutMode mode,
                                                               const ShapedString::Paragraph &spara,
...
static Float64 minYOfSpacingBelowFirstBaselineInNewParagraph(STUTextLayoutMode mode,
                                                             const STUTextFrameLine &line,
                                                             const ShapedString::Paragraph &para,
                                                             const ShapedString::Paragraph &nextPara)
{
  ...
  if (minBaselineDistance == 0) {
    ...
    offset += extraSpacingBeforeFirstAndAfterLastLineInParagraphDueToMinBaselineDistance(line, minBaselineDistance);
    minBaselineDistance = max(minBaselineDistance, nextPara.minBaselineDistance);
  }
  if (minBaselineDistance == 0 || nextPara.firstLineOffsetType != STUOffsetOfFirstBaselineFromDefault) {
    return line.originY + offset;
  }
  const Float32 offset2 =
      minBaselineDistance + nextPara.firstLineOffset + nextPara.effectiveMinHeightBelowBaselineWithoutSpacing(mode);
    ...
    STUTextLayoutMode mode, const ShapedString::Paragraph *para, Int32 stringEndIndex)
{
  ...
  if (para->stringRange.end < stringEndIndex) {
    ...
    ++para;
    Float32 d1 = p + para->paddingTop + minDistanceFromParagraphTopToSpacingBelowFirstBaseline(mode, *para, none);
    if (para->firstLineOffsetType == STUOffsetOfFirstBaselineFromDefault) {
      const Float32 d2 =
          max(minBaselineDistance, para->minBaselineDistance) + para->firstLineOffset +
          (para->effectiveMinHeightBelowBaselineWithoutSpacing(mode) - mh.minHeightBelowBaselineWithoutSpacing);
      d1 = max(d1, d2);
    }
    d = min(d, d1);
    if (tsi >= 0 && tsi == para->truncationScopeIndex) {
      ...
      while (para->stringRange.end < stringEndIndex) {
        ...
        if (para->truncationScopeIndex != tsi) {
          d1 = p + para->paddingTop + minDistanceFromParagraphTopToSpacingBelowFirstBaseline(mode, *para, none);
          if (para->firstLineOffsetType == STUOffsetOfFirstBaselineFromDefault) {
            const Float32 d2 =
                max(minBaselineDistance, para->minBaselineDistance) + para->firstLineOffset +
                (para->effectiveMinHeightBelowBaselineWithoutSpacing(mode) - mh.minHeightBelowBaselineWithoutSpacing);
...
STU_INLINE
bool isLeftAligned(const STUTextFrameParagraph &para) { return para.alignment <= STUParagraphAlignmentJustifiedLeft; }

STU_INLINE
bool isJustified(const STUTextFrameParagraph &para) { return (para.alignment & 0x1) != 0; }

bool TextFrameLayouter::lastLineFitsFrameHeight() const
{
  if (STU_UNLIKELY(lines_.isEmpty()))
    return true;
                               ...
                               const TextFrameOptions &options)
{
  ...
  if (STU_UNLIKELY(paras_.isEmpty()))
    return;
  if (!lines_.isEmpty()) {
    STU_ASSERT(ownsCTLinesAndParagraphTruncationTokens_);
    static_assert(isTriviallyDestructible<TextFrameLine>);
  ...
  Float64 minYOfSpacingBelowBaseline =
      minDistanceFromParagraphTopToSpacingBelowFirstBaseline(layoutMode_, *spara, scaleInfo_);
NewTruncationScope:;
...
NewParagraph:;
  ...
  if (__builtin_add_overflow(para->lineIndexRange.start, spara->maxNumberOfInitialLines, &para->initialLinesEndIndex)) {
    para->initialLinesEndIndex = maxValue<Int32>;
  }
  const Float64 minLineDistance = maxLineCount <= lines_.count() + 1
                                      ? 0
                                      : minDistanceFromMinYOfSpacingBelowBaselineToMinYOfSpacingBelowNextBaseline(
                                            layoutMode_, spara, stringRange_.end);
  for (;;) {
    if (maxLineCount <= lines_.count() + 1 || minYOfSpacingBelowBaseline + minLineDistance > frameHeightPlusEpsilon) {
    LastLine:
    ...
    line->init_step1(TextFrameLine::InitStep1Params{
        ...
        .rangeInTruncatedStringStart =
            isFirstLineInParagraph
                ? (para->paragraphIndex == 0 ? 0 : para[-1].rangeInTruncatedString.end)
                : line[-1].rangeInTruncatedString.end + line[-1].trailingWhitespaceInTruncatedStringLength,
    ...
    ShouldTruncate shouldTruncate = isLastLineInFrame && lastLineTruncationMode != STULastLineTruncationModeClip
                                        ? shouldTruncate_withoutTruncationScope
                                        : shouldNotTruncate;
    if (truncationScope &&
    ...
    if (!shouldTruncate) {
      ...
      nextStringIndex = line->rangeInOriginalString.end + line->trailingWhitespaceInTruncatedStringLength;
    } else {
      ...
      if (shouldTruncate == shouldTruncate_withoutTruncationScope) {
        ...
        case STULastLineTruncationModeStart:
          mode = kCTLineTruncationStart;
          break;
        case STULastLineTruncationModeMiddle:
          mode = kCTLineTruncationMiddle;
          break;
        default:
          mode = kCTLineTruncationEnd;
          break;
        }
        ...
        if (truncationScope->stringRange.end < stringRange_.end) {
          nextStringIndex = truncationScope->stringRange.end - truncationScope->finalLineTerminatorUTF16Length;
        } else {
          nextStringIndex = stringRange_.end;
        }
      }
      truncateLine(*line,
                   ...
                   mode,
                   token,
                   options.truncationRangeAdjuster,
                   *para,
                   tokenStyleBuffer_);
    ...
    if ((lastLineTruncationMode != STULastLineTruncationModeClip || lines_.count() != maxLineCount) &&
      ...
      if (stringIndex < para->rangeInOriginalString.end) {
        minYOfSpacingBelowBaseline = minYOfSpacingBelowNextBaselineInSameParagraph(layoutMode_, *line, *spara);
        if (minYOfSpacingBelowBaseline <= frameHeightPlusEpsilon)
          continue;
        if (lastLineTruncationMode != STULastLineTruncationModeClip)
          goto BacktrackOneLine;
        goto ClipPara;
      }
      ...
      if (__builtin_add_overflow(
              para->lineIndexRange.start, spara->maxNumberOfInitialLines, &para->initialLinesEndIndex)) {
        para->initialLinesEndIndex = maxValue<Int32>;
      }
      hyphenationFactor_ = spara->hyphenationFactor;
    ...
    }
    {
      ...
      const Int32 end = Range{para->rangeInOriginalString}.count() - Range{para->excisedRangeInOriginalString}.count() +
                        para->truncationTokenLength + start;
    ...
    needToJustifyLines_ |= isJustified(*para) && para->lineIndexRange.start + 1 < para->lineIndexRange.end;
    if (STU_UNLIKELY(clipped))
      break;
    ...
    for (;;) {
      ...
      ++para;
      ++spara;
      if (stringIndex == para->rangeInOriginalString.start)
        break;
      ...
      STU_ASSERT(stringIndex >= para->rangeInOriginalString.end - para->paragraphTerminatorInOriginalStringLength);
      ...
      const Int32 remainingTerminatorLength = para->rangeInOriginalString.end - para->excisedRangeInOriginalString.end;
    ...
    minYOfSpacingBelowBaseline =
        minYOfSpacingBelowFirstBaselineInNewParagraph(layoutMode_, *line, *previousSPara, *spara);
    if (minYOfSpacingBelowBaseline <= frameHeightPlusEpsilon) {
      if (spara->truncationScopeIndex == previousSPara->truncationScopeIndex)
        goto NewParagraph;
...
static FontMetricsAndStyleFlags calculateOriginalFontsMetricsForLineRange(STUStartEndRangeI32 range,
                                                                          ...
                                                                          const FontMetrics *__nonnull fontMetrics)
{
  ...
  FontMetrics metrics =
      !style->hasAttachment() ? fontMetrics[style->fontIndex().value] : style->attachmentInfo()->attribute->_metrics;
  ...
  for (;;) {
    ...
    if (stringIndex >= range.end)
      break;
    style = next;
    flags |= style->flags();
    const FontMetrics &styleMetrics =
        !style->hasAttachment() ? fontMetrics[style->fontIndex().value] : style->attachmentInfo()->attribute->_metrics;
...
const TextStyle *TextFrameLayouter::initializeTypographicMetricsOfLine(TextFrameLine &line)
{
  ...
  const TextStyle *lastOriginalStringStyle;
  {
    ...
    if (!stringRange1.isEmpty()) {
      const FontMetricsAndStyleFlags fsi =
          calculateOriginalFontsMetricsForLineRange(stringRange1, style, originalStringFontMetrics_.begin());
      style = fsi.nextStyle;
    ...
    if (!stringRange2.isEmpty()) {
      const FontMetricsAndStyleFlags fsi =
          calculateOriginalFontsMetricsForLineRange(stringRange2, style, originalStringFontMetrics_.begin());
      style = fsi.nextStyle;
      nonTokenTextFlags |= fsi.flags;
  ...
  if (line.hasTruncationToken) {
    ...
    const FontMetricsAndStyleFlags fsi =
        calculateOriginalFontsMetricsForLineRange(Range{0, tokenLength}, tokenStyles, tokenFontMetrics_.begin());
    metrics.aggregate(fsi.metrics);
  ...
  { // Adjust the font info to account for substituted fonts, check for color glyphs
    ...
    down_cast<const TextFrameLine &>(line).forEachGlyphSpan(
        [&](const TextLinePart part, CTLineXOffset, GlyphSpan glyphSpan) {
          ...
          if (part == TextLinePart::originalString) {
            ...
            if (tokenPartHasBaselineOffsetOrAttachment) {
              if (part == TextLinePart::truncationToken) {
                ...
                style =
                    reinterpret_cast<const TextStyle *>(originalStringStyles_.dataBegin() + line._tokenStylesOffset);
          ...
          if (font == previousFont && baselineOffset == previousBaselineOffset)
            return;
