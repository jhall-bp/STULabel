...
struct IsTruncationTokenRange : Parameter<IsTruncationTokenRange>
{
  using Parameter::Parameter;
};

STU_INLINE
ShouldStop forEachStyledStringRangeImpl(Range<Int32> stringRange,
                                        Int32 offsetInTruncatedString,
                                        ...
                                        FunctionRef<ShouldStop(const TextStyle &, StyledStringRange)> body)
{
  ...
  for (;;) {
    ...
    if (shouldStop || nextIndex == stringRange.end)
      break;
    style = &next;
    index = nextIndex;
  }
                                        ...
                                        FunctionRef<ShouldStop(const TextStyle &, StyledStringRange)> body)
{
  ...
  const Int32 i1 = !styleOverride ? stringRange.end : max(stringRange.start, overriddenStringRange.start);
  ShouldStop shouldStop = forEachStyledStringRangeImpl(
      Range{stringRange.start, i1}, offsetInTruncatedString, isTruncationTokenRange, inOutStyle, nil, body);
  if (!shouldStop && styleOverride) {
    ...
    shouldStop = forEachStyledStringRangeImpl(
        Range{i1, i2}, offsetInTruncatedString, isTruncationTokenRange, inOutStyle, styleOverride, body);
    if (!shouldStop) {
      shouldStop = forEachStyledStringRangeImpl(
          Range{i2, stringRange.end}, offsetInTruncatedString, isTruncationTokenRange, inOutStyle, nil, body);
    }
  }
  return shouldStop;
}
...
STU_NO_INLINE
ShouldStop forEachStyledStringRange(const TextFrame &textFrame,
                                    ...
                                    FunctionRef<ShouldStop(const TextStyle &, StyledStringRange)> body)
{
  ...
  const Range<Int32> range1 = {firstLine.rangeInOriginalString.start, paragraph.excisedRangeInOriginalString().start};
  const Range<Int32> range2 = {paragraph.excisedRangeInOriginalString().end, lastLine.rangeInOriginalString.end};
  const Int32 range1OffsetInTruncatedString =
      firstLine.rangeInTruncatedString.start - firstLine.rangeInOriginalString.start;
  const Int32 range2OffsetInTruncatedString = lastLine.rangeInTruncatedString.end - lastLine.rangeInOriginalString.end;
  ...
  ShouldStop shouldStop = forEachStyledStringRangeImpl(range1,
                                                       range1OffsetInTruncatedString,
                                                       IsTruncationTokenRange{false},
                                                       InOut{style},
                                                       styleOverride,
                                                       drawnRangeInTruncatedString,
                                                       overrideRangeInTruncatedString,
                                                       body);
  if (!shouldStop && STU_UNLIKELY(range1.end < range2.start)) {
    shouldStop = forEachStyledStringRangeImpl({0, paragraph.truncationTokenLength},
                                              paragraph.rangeOfTruncationTokenInTruncatedString().start,
                                              IsTruncationTokenRange{true},
                                              InOut{tokenStyle},
                                              styleOverride,
                                              drawnRangeInTruncatedString,
                                              overrideRangeInTruncatedString,
                                              body);
    if (!shouldStop) {
      shouldStop = forEachStyledStringRangeImpl(range2,
                                                range2OffsetInTruncatedString,
                                                IsTruncationTokenRange{false},
                                                InOut{style},
                                                styleOverride,
                                                drawnRangeInTruncatedString,
