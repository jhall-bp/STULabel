...
struct STUTextRectArrayData
{
  ...
  {
    return sizeof(STUTextRectArrayData) + sign_cast(rectCount) * sizeof(TextLineSpan) +
           sign_cast(lineCount) * sizeof(TextLineVerticalPosition);
  }

  UInt sizeInBytes() const { return STUTextRectArrayData::sizeInBytes(rectCount, lineCount); }

  NSRange textLineRange() const { return {sign_cast(textLineIndexOffset), sign_cast(lineCount)}; };
  ...
  bool operator==(const STUTextRectArrayData &other)
  {
    ...
    if (rectCount != other.rectCount)
      return false;
    if (textLineRange() != other.textLineRange())
      return false;
    for (Int32 i = 0; i < rectCount; ++i) {
      const TextLineSpan &span = spanArray[i];
      const TextLineSpan &otherSpan = other.spanArray[i];
      if (span.x != otherSpan.x)
        return false;
      if (span.isLeftEndOfLine != otherSpan.isLeftEndOfLine)
        return false;
      if (span.isRightEndOfLine != otherSpan.isRightEndOfLine)
        return false;
      if (span.lineIndex != otherSpan.lineIndex)
        return false;
    }
...
struct DataOrOtherArray
{
  ...
  explicit STU_INLINE DataOrOtherArray(const STUTextRectArray *__unsafe_unretained array)
  {
...
static STU_INLINE Class stuTextRectArrayClass()
{
...
+ (STUTextRectArray *)emptyArray
{
  STU_STATIC_CONST_ONCE(
      STUTextRectArray *,
      instance,
      STUTextRectArrayCreate(stuTextRectArrayClass(), {}, {}, TextFrameOrigin{}, TextFrameScaleAndDisplayScale::one()));
...
STUTextRectArray *__nonnull STUTextRectArrayCreate(__nullable Class cls,
                                                   ...
                                                   const TextFrameScaleAndDisplayScale &scales) NS_RETURNS_RETAINED
{
  if (!cls) {
  ...
  const Int32 lineCount = spans.isEmpty() ? 0 : spans[$ - 1].lineIndex - spans[0].lineIndex + 1;
  ...
  if (spans.isEmpty()) {
    ...
    const Float64 yOffset =
        scales.textFrameScale == 1 ? frameOrigin.value.y : frameOrigin.value.y / scales.textFrameScale;
    for (Int i = 0; i < vpArray.count(); ++i) {
      ...
      TextLineVerticalPosition vp = textLineVerticalPosition(
          line, scales.displayScale, VerticalEdgeInsets{}, VerticalOffsets{.textFrameOriginY = yOffset});
...
STUTextRectArray *__nonnull STUTextRectArrayCopyWithOffset(Class cls,
                                                           const STUTextRectArray *__unsafe_unretained array,
                                                           CGPoint offset) NS_RETURNS_RETAINED
{
...
#define CHECK_RECT_INDEX(data, index)                                                                                  \
  STU_CHECK_MSG(static_cast<UInt>(index) < sign_cast((data).rectCount), "The rect index is out of bounds.")
...
STUIndexAndDistance STUTextRectArrayFindRectClosestToPoint(const STUTextRectArray *__unsafe_unretained self,
                                                           CGPoint point,
                                                           CGFloat maxDistance)
{
  const DataOrOtherArray d{self};
  if (d.data) {
    ...
    if (rectCount > 0 && data.bounds.squaredDistanceTo(point) <= maxSquaredDistance) {
      ...
      if (minSquaredDistance <= maxSquaredDistance) {
        return {.index = sign_cast(minIndex), .distance = sqrt(narrow_cast<CGFloat>(minSquaredDistance))};
      }
    }
                                  ...
                                  transform:(nullable const CGAffineTransform *)transform CF_RETURNS_RETAINED
{
  ...
  if (d.data) {
    ...
    addLineSpansPath(*path,
                     data.spans(),
                     data.textLineVerticalPositions(),
