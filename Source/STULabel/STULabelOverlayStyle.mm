...
#define FOR_ALL_FIELDS(f)                                                                                              \
  f(bool, extendTextLinesToCommonHorizontalBounds) f(UIEdgeInsets, edgeInsets) f(CGFloat, cornerRadius)                \
      f(CGFloat, borderWidth) f(CFTimeInterval, fadeInDuration) f(CFTimeInterval, fadeOutDuration) f(UIColor *, color) \
          f(UIColor *, borderColor) f(CAMediaTimingFunction *, fadeInTimingFunction)                                   \
...
- (BOOL)isEqual:(id)object
{
  if (self == object)
    return true;
  if (![object isKindOfClass:STULabelOverlayStyle.class])
    return false;
  STULabelOverlayStyle *const other = object;
  return equal(_color, other->_color) && _cornerRadius == other->_cornerRadius &&
         _extendTextLinesToCommonHorizontalBounds == other->_extendTextLinesToCommonHorizontalBounds &&
         UIEdgeInsetsEqualToEdgeInsets(_edgeInsets, other->_edgeInsets) && _borderWidth == other->_borderWidth &&
         equal(_borderColor, other->_borderColor) && _fadeInDuration == other->_fadeInDuration &&
         _fadeOutDuration == other->_fadeOutDuration &&
         stu_CAMediaTimingFunctionEqualToFunction(_fadeInTimingFunction, other->_fadeInTimingFunction) &&
         stu_CAMediaTimingFunctionEqualToFunction(_fadeOutTimingFunction, other->_fadeOutTimingFunction);
}
...
{
  STULabelOverlayStyleBuilder *const builder = [[STULabelOverlayStyleBuilder alloc] initWithStyle:self];
  block(builder);
