...
- (bool)hasShapedText
{
  return prerenderer->hasShapedString();
}
...
- (CGSize)size
{
  return prerenderer->size();
}

- (STULabelPrerendererSizeOptions)sizeOptions
{
  return prerenderer->sizeOptions();
}

- (UIEdgeInsets)contentInsets
{
  return prerenderer->contentInsets();
}
...
{
  prerenderer->setSizeAndContentInsets(CGSize{width, maxHeight}, contentInsets, STUShrinkLabelHeightToFit);
}
...
{
  prerenderer->setSizeAndContentInsets(
      CGSize{maxWidth, maxHeight}, contentInsets, STUShrinkLabelWidthToFit | STUShrinkLabelHeightToFit);
}
...
- (bool)hasLayoutInfo
{
  return prerenderer->hasLayoutInfo();
}

- (STULabelLayoutInfo)layoutInfo
{
  return prerenderer->layoutInfo();
}

- (CGSize)sizeThatFits
{
  return prerenderer->sizeThatFits();
}

- (bool)tryGetSizeThatFits:(nullable CGSize *)outSizeThatFits layoutInfo:(nullable STULabelLayoutInfo *)outLayoutInfo
{
...
{
  return prerenderer->links().unretained;
}

- (bool)hasTextFrame
{
  return prerenderer->hasTextFrame();
}

- (STUTextFrame *)textFrame
{
  return prerenderer->textFrame().unretained;
}
