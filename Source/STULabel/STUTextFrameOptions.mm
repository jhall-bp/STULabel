...
#define FOR_ALL_FIELDS(f)                                                                                              \
  f(NSInteger, maximumNumberOfLines) f(STUTextLayoutMode, textLayoutMode)                                              \
      f(STUDefaultTextAlignment, defaultTextAlignment) f(STULastLineTruncationMode, lastLineTruncationMode)            \
          f(NSAttributedString *__nullable, truncationToken)                                                           \
              f(__nullable STUTruncationRangeAdjuster, truncationRangeAdjuster) f(CGFloat, minimumTextScaleFactor)     \
                  f(CGFloat, textScaleFactorStepSize) f(STUBaselineAdjustment, textScalingBaselineAdjustment)          \
...
#define DEFINE_GETTER(Type, name)                                                                                      \
  -(Type)name { return _options.name; }
...
STUTextFrameOptions *STUTextFrameOptionsCopy(STUTextFrameOptions *__unsafe_unretained options) NS_RETURNS_RETAINED
{
...
{
  STUTextFrameOptionsBuilder *const builder = [[STUTextFrameOptionsBuilder alloc] initWithOptions:self];
  block(builder);
