...
- (stu_label::Rect<CGFloat>)checkGlyphBoundsWithFont:(UIFont *)font
                                               glyph:(CGGlyph)glyph
                                               cache:(FontFaceGlyphBoundsCache &)cache
{
  ...
  const stu_label::Rect<CGFloat> r2 =
      CTFontGetBoundingRectsForGlyphs((__bridge CTFontRef)font, kCTFontOrientationHorizontal, &glyph, nullptr, 1);
  if (r2.isEmpty()) {
    XCTAssert(r1 == stu_label::Rect<CGFloat>{});
    return r1;
  }
  const CGFloat eps = cache.usesIntBounds() ? epsilon<CGFloat> * (isSame<CGFloat, Float32> ? 2 : 1) : epsilon<Float32>;
  XCTAssertEqualWithAccuracy(r1.x.start, r2.x.start, eps * abs(r2.x.start), @"font: %@ glyph: %i", font, glyph);
  XCTAssertEqualWithAccuracy(r1.y.start, r2.y.start, eps * abs(r2.y.start), @"font: %@ glyph: %i", font, glyph);
  XCTAssertEqualWithAccuracy(
      r1.x.end, r2.x.end, eps * max(abs(r2.x.end), r2.width()), @"font: %@ glyph: %i", font, glyph);
  XCTAssertEqualWithAccuracy(
      r1.y.end, r2.y.end, eps * max(abs(r2.y.end), r2.height()), @"font: %@ glyph: %i", font, glyph);
  return r1;
}
                 ...
                 maxRelativeError:(CGFloat)maxRelativeError
{
  ...
  while (randomGlyphCount > 0) {
    ...
    [self checkBoundingRectWithFont:font
                             glyphs:glyphs
                          positions:positions
                              cache:cache
...
{
  NSAttributedString *const string = [[NSAttributedString alloc] initWithString:@"😎"
  ...
  CTFont *const runFont = static_cast<CTFont *>(CFDictionaryGetValue(CTRunGetAttributes(run), kCTFontAttributeName));
...
- (void)testIndividualGlyphBounds
{
  ...
  for (const auto &category : categories) {
    ...
    for (const auto &textStyle : textStyles) {
      [self testBoundsOfAllGlyphsOfFont:[UIFont preferredFontForTextStyle:textStyle compatibleWithTraitCollection:tc]];
    }
...
- (void)testBoundingRectWithRandomGlyphs
{
  ...
  const auto getFreshCache = [&](UIFont *font) {
    ...
    FontFaceGlyphBoundsCache::exchange(InOut(cache), largerFont, FontFace{largerFont, largerFont.pointSize});
  ...
  };
  {
    ...
    [self checkBoundingRectWithFont:font randomGlyphCount:100000 cache:*cache maxRelativeError:0];
  }
  {
  ...
  }
  {
    ...
    UIFont *font = (__bridge UIFont *)CTFontCreateWithName((__bridge CFStringRef) @"HelveticaNeue", 17, &matrix);
    auto cache = getFreshCache(font);
    [self checkBoundingRectWithFont:font randomGlyphCount:100000 cache:*cache maxRelativeError:0];
  }
}
...
- (void)testFallbackToFloatBoundsWithFont:(UIFont *)font string:(NSString *)string
{
  ...
  CTLine *const line = CTLineCreateWithAttributedString((__bridge CFAttributedStringRef)attributedString);
...

  {
    ...
    [self checkBoundingRectWithFont:runFont glyphs:glyphs positions:positions cache:*cache maxRelativeError:0];
    [self checkBoundingRectWithFont:runFont randomGlyphCount:10000 cache:*cache maxRelativeError:epsilon<Float32> / 2];
  }

  {
    const auto cache = getFreshCache();
    cache->setMaxIntBoundsCountToTestFallbacktToFloatBounds(3);
    [self checkBoundingRectWithFont:runFont glyphs:glyphs[{0, 2}] positions:positions cache:*cache maxRelativeError:0];
    [self checkBoundingRectWithFont:runFont
                             glyphs:glyphs
                          positions:positions
                              cache:*cache
