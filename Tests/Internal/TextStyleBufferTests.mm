...
static NSDictionary<NSAttributedStringKey, id> *lotsOfAttributes()
{
  ...
  const auto s = [[NSShadow alloc] init];
...
- (void)testLargeTextStyle
{
  ...
  NSMutableDictionary<NSAttributedStringKey, id> *NS_VALID_UNTIL_END_OF_SCOPE attributes1b = [attributes1 mutableCopy];
  NSMutableDictionary<NSAttributedStringKey, id> *NS_VALID_UNTIL_END_OF_SCOPE attributes2 = [attributes1 mutableCopy];
  ...
  const auto flags = TextFlags::hasLink | TextFlags::hasBackground | TextFlags::hasShadow | TextFlags::hasUnderline |
                     TextFlags::hasStrikethrough | TextFlags::hasStroke | TextFlags::hasAttachment |
                     TextFlags::hasBaselineOffset | TextFlags::mayNotBeGrayscale | TextFlags::usesExtendedColor;
  ...
  const NSUnderlineStyle underlineStyle = ((NSNumber *)attributes1[NSUnderlineStyleAttributeName]).integerValue;
  UIColor *const underlineColor = attributes1[NSUnderlineColorAttributeName];

  const NSUnderlineStyle strikethroughStyle = ((NSNumber *)attributes1[NSStrikethroughStyleAttributeName]).integerValue;
  ...
  for (const TextStyle *s = &s0; s != &s->next(); s = &s->next()) {
    ...
    XCTAssertEqual(bufferColor(*s->backgroundInfo()->colorIndex), Color{background.color});
    XCTAssertEqual(bufferColor(*s->backgroundInfo()->borderColorIndex), Color{background.borderColor});
...
- (void)testColorOverflowHandling
{
  ...
  NSMutableArray *NS_VALID_UNTIL_END_OF_SCOPE const colors = [[NSMutableArray alloc] initWithCapacity:n];
...
- (void)testFontOverflowHandling
{
  ...
  NSMutableArray<UIFont *> *NS_VALID_UNTIL_END_OF_SCOPE const fonts = [[NSMutableArray alloc] initWithCapacity:n];
  ...
  for (int i = 0; i < TextStyleBuffer::maxFontCount; ++i, s = &s->next()) {
    ...
    XCTAssertEqual(buffer.fonts()[s->fontIndex().value].ctFont(), (__bridge CTFont *)fonts[sign_cast(i)]);
  }
