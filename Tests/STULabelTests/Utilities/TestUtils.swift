...
func createImage(
  _ size: CGSize, scale: CGFloat, backgroundColor: UIColor? = nil,
  _ format: STUCGImageFormat.Predefined,
  _ closure: @convention(block) (CGContext) -> Void
)
  -> UIImage
{
  ...
  let cgImage = stu_createCGImage(
    size: size, scale: scale, backgroundColor: backgroundColor?.cgColor,
