...
struct TextFrameLayoutInfoTests {
  private func expectEqual(
    _ actual: CGFloat, _ expected: CGFloat, accuracyInULP: CGFloat,
    sourceLocation: SourceLocation = #_sourceLocation
  ) {
    #expect(abs(actual - expected) <= accuracyInULP * expected.ulp, sourceLocation: sourceLocation)
  }

  private func expectEqual(
    _ actual: CGFloat, _ expected: CGFloat,
    accuracyInFloat32ULP: CGFloat, sourceLocation: SourceLocation = #_sourceLocation
  ) {
    #expect(
      abs(actual - expected) <= accuracyInFloat32ULP * CGFloat(Float32(expected).ulp),
  ...
  private func assertLayoutInfoConsistency(_ tf: STUTextFrame, origin: CGPoint) {
    ...
    for info in [info1, info2, info2b] {
      ...
      #expect(
        info0.lastLineHeightBelowBaselineWithoutSpacing
          == info.lastLineHeightBelowBaselineWithoutSpacing)
      #expect(
        info0.lastLineHeightBelowBaselineWithMinimalSpacing
    ...
    #expect(
      CGFloat(info0.firstBaseline) == tf.firstBaseline(frameOriginY: origin.y, displayScale: nil))
    #expect(
      CGFloat(info0.lastBaseline) == tf.lastBaseline(frameOriginY: origin.y, displayScale: nil))
    #expect(CGFloat(info1.firstBaseline) == tf.firstBaseline(frameOriginY: origin.y))
    #expect(CGFloat(info1.lastBaseline) == tf.lastBaseline(frameOriginY: origin.y))
    #expect(
      CGFloat(info2.firstBaseline) == tf.firstBaseline(frameOriginY: origin.y, displayScale: s))
    #expect(CGFloat(info2.lastBaseline) == tf.lastBaseline(frameOriginY: origin.y, displayScale: s))
    ...
    for line in tf.lines {
      #expect(line.baselineOrigin == line.baselineOrigin(textFrameOrigin: .zero, displayScale: nil))

      #expect(
        line.typographicBounds == line.typographicBounds(textFrameOrigin: .zero, displayScale: nil))
      ...
      expectApproximatelyEqual(minX, bounds0.origin.x, tolerance: minX.ulp)
      let minY = origin.y + bo.y - CGFloat((Float32(ascent) + Float32(leading) / 2))
      expectApproximatelyEqual(minY, bounds0.origin.y, tolerance: 8 * minY.ulp)

      expectApproximatelyEqual(width, bounds0.size.width, tolerance: width.ulp)
      let height = CGFloat(Float32(ascent) + Float32(descent) + Float32(leading))
      expectApproximatelyEqual(height, bounds0.size.height, tolerance: height.ulp)

      expectApproximatelyEqual(minX, bounds2.origin.x, tolerance: minX.ulp)
      expectApproximatelyEqual(width, bounds2.size.width, tolerance: width.ulp)
      expectApproximatelyEqual(height, bounds2.size.height, tolerance: height.ulp)

      let minY2 =
        ceilToScale(origin.y + bo.y, s) - CGFloat((Float32(ascent) + Float32(leading) / 2))
      expectApproximatelyEqual(minY2, bounds2.origin.y, tolerance: minY2.ulp)
    }
    ...
    if let ts = tf.displayScale {
      #expect(
        tf.firstBaseline(frameOriginY: origin.y)
          == ceilToScale(
            origin.y + tf.firstBaseline(frameOriginY: 0, displayScale: nil),
            ts))
      #expect(
        tf.lastBaseline(frameOriginY: origin.y)
          == ceilToScale(
            origin.y + tf.lastBaseline(frameOriginY: 0, displayScale: nil),
            ts))
    }
    ...
    for i in tf.lines.indices {
      ...
      #expect(
        a.trailingWhitespaceInTruncatedStringUTF16Length
      ...
      #expect(a.baselineOrigin(textFrameOrigin: origin, displayScale: s) == b.baselineOrigin)
      #expect(a.width == b.width)
      #expect(a.ascent == b.ascent)
      #expect(a.descent == b.descent)
      #expect(a.leading == b.leading)
      #expect(a.typographicBounds(textFrameOrigin: origin, displayScale: s) == b.typographicBounds)
  ...
  func `Layout info`() {
    ...
    ({
      ...
      let width =
        STUTextFrame(
          STUShapedString(text), size: CGSize(width: 200, height: 100),
          displayScale: 0, options: nil
        ).layoutBounds.size.width / 2
      let tf = STUTextFrame(
        STUShapedString(text), size: CGSize(width: width, height: 100),
        displayScale: 3,
      ...
      expectEqual(
        tf.firstLineHeightAboveBaseline, s * font1.ascender,
        accuracyInFloat32ULP: 1)

      expectEqual(
        tf.lastLineHeightBelowBaseline, s * (-font1.descender + g),
        accuracyInFloat32ULP: 1)
      ...
      expectEqual(
        tf.lastLineHeightBelowBaselineWithoutSpacing, s * (-font1.descender),
        accuracyInFloat32ULP: 1)

      expectEqual(tf.firstBaseline, ceilToScale(s * font1.ascender, 3), accuracyInULP: 1)

      expectEqual(
        tf.lines.first!.ascent, s * max(font1.ascender, font1b.ascender),
        accuracyInFloat32ULP: 1)

      expectEqual(
        tf.lines.first!.descent, s * max(-font1.descender, -font1b.descender),
        accuracyInFloat32ULP: 1)

      expectEqual(
        tf.lines.first!.leading, s * max(font1.leading, font1b.leading),
        accuracyInFloat32ULP: 1)
    ...
    ({
      ...
      expectEqual(
        tf.lastLineHeightBelowBaseline, s * (-font2.descender + g),
        accuracyInFloat32ULP: 1)
      ...
      expectEqual(
        tf.lastLineHeightBelowBaselineWithoutSpacing, s * (-font2.descender),
        accuracyInFloat32ULP: 1)

      expectEqual(tf.firstBaseline, ceilToScale(s * font1.ascender, 3), accuracyInULP: 1)

      expectEqual(
        tf.lastBaseline, ceilToScale(s * (font1LineHeight + font2.ascender), 3),
        accuracyInULP: 1)
      ...
      expectEqual(
        tf.lines.last!.ascent, s * max(font2.ascender, font2b.ascender),
        accuracyInFloat32ULP: 1)

      expectEqual(
        tf.lines.last!.descent, s * max(-font2.descender, -font2b.descender),
        accuracyInFloat32ULP: 1)

      expectEqual(
        tf.lines.last!.leading, s * max(font2.leading, font2b.leading),
        accuracyInFloat32ULP: 1)
