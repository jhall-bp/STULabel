...
- (STULabel*)labelWithAttributedText:(NSAttributedString*)text
                                size:(CGSize)size
...
- (STULabel*)labelWithText:(NSString*)text size:(CGSize)size {
  return [self labelWithAttributedText:[[NSAttributedString alloc]
                                  ...
                                  size:size insets:UIEdgeInsetsZero];
}
...
- (CGPoint)pointInLabel:(STULabel*)label range:(NSRange)range {
  ...
  STUTextRectArray* const rects = [textFrame
    rectsForRange:[textFrame rangeForRangeInTruncatedString:range]
...
- (void)testPositionsAndRangesHaveValueSemantics {
  ...
  UITextRange* const firstRange = [firstInput textRangeFromPosition:firstPosition
...
- (void)testVisibleStringPositionsAndComposedCharacterBoundaries {
  ...
  XCTAssertNil([input positionFromPosition:afterA offset:1]);
...
- (void)testMultilineGeometryHitTestingAndTokenizer {
  STULabel* const label = [self labelWithAttributedText:[[NSAttributedString alloc]
  ...
  UITextRange* const wordRange = [input.tokenizer
    rangeEnclosingPosition:input.beginningOfDocument
  ...
  UITextPosition* const nextLine = [input positionFromPosition:input.beginningOfDocument
                                                    inDirection:UITextLayoutDirectionDown offset:1];
...
- (void)testTruncationTokenLinkCannotBeSelected {
  ...
  NSMutableAttributedString* const token =
    [[NSMutableAttributedString alloc] initWithString:@"… more"
                                            attributes:@{NSFontAttributeName: font}];
  [token addAttribute:NSLinkAttributeName value:@"more" range:NSMakeRange(2, 4)];
  STULabel* const label = [self labelWithAttributedText:[[NSAttributedString alloc]
                                    initWithString:@"A long sentence that needs truncation."
                                        attributes:@{NSFontAttributeName: font}]
  ...
  XCTAssertEqualObjects([input textInRange:selection],
                        [label.textFrame.truncatedAttributedString.string
                          substringToIndex:link.rangeInTruncatedString.location]);
